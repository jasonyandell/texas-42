...
fn scalar_solver_agrees_with_the_symbolic_operator_at_trick6() {
    ...
    let directions = [
        ...
        (ScalarValuation::trick_plus_count(), {
            let mut base = walt_geom::FeatureVec::unit_trick();
            for d in Domino::ALL {
                base.tiles[d.index()] = qi(i128::from(d.count()));
            }
            Direction {
                base,
                delta: walt_geom::FeatureVec::zero(),
            }
        }),
    ];
...
fn sampled_census(
    ...
    walt_strat::scalar_census(kernel, val, worlds.iter().copied())
}
...
fn true_world_vector(r: &Receipt, kernel: &Kernel, hand: usize, val: ScalarValuation) -> Vec<i64> {
    let (true_hands, _) =
        walt_core::replay::state_before_trick(&r.hands[hand], 3).expect("a valid hand");
