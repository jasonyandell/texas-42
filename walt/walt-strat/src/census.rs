...
use walt_geom::{argmax_correspondence, ArgmaxCorrespondence, Envelope, Q};
use walt_kernel::Kernel;

use crate::direction::Direction;
use crate::pi::pi_root_values;
...
pub fn pi_census(kernel: &Kernel, focal: Team, dir: &Direction) -> PiCensus {
    ...
    for world in kernel.worlds() {
        ...
        let baseline: Vec<Q> = envelopes.iter().map(|e| e.eval(walt_geom::qi(0))).collect();
