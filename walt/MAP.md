...
| gate wall (`check.sh`) | 308 s with FH3's anchors gate (230 s before it; 367 s when the concurrent runner was first wired at e53752b5, before CI1's fixtures; before the runner the serial test stage summed 498 s of suite walls plus FH1's 350 s suite — `briefs/BRIEF-CI1.md`); the anchors suite needed 18.2 GB before FH5's in-flight cap and 8.8 GB after it (b6de5a25), and the runner's nine processes summed up to ~21 GB at the FH4 audit | fixed by concurrency + fixtures; corpus trimming still owed ([[gate-corpus-trim]]); NOT run on the 2026-09-06/07 commits (session waiver) |
