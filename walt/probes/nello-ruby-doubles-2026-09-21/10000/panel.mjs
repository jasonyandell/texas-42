...
const worlds = Number(process.argv[3] ?? '10000');
if (!source || ![2000, 10000].includes(worlds)) throw new Error('usage: node panel.mjs WASM [2000|10000]');
...
const prior = JSON.parse(readFileSync(dir + '2000/panel2000.json')).rows;
...
for (const pos of positions) for (const seed of [1,2,3,4,5,6,7,8]) {
  ...
  if (worlds === 2000) {
    ...
    row.parity_with_panel2000 = {options: true, choice: true};
  }
  rows.push(row);
