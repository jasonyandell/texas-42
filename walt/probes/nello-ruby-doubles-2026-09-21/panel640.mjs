// Same frozen preview WASM and public/own-hand inputs as the 160-world probe.
...
const previous=JSON.parse(readFileSync(dir+'/panel.json')).rows;
...
for(const pos of positions) for(const seed of [1,2,3,4,5,6,7,8]) {
  ...
  const call={request:{...pos.request,seed},worlds:640,partner:false,budget_ms:20000};
  ...
  assert.equal(result.evaluation.outer_worlds,640);
  assert.equal(result.route,'baseline');
  assert.ok(result.phases.every(p=>p.status==='completed'));
  const old=previous.find(r=>r.ply===pos.ply && r.call.request.seed===seed);
  const baseline40=checkpoints.find(c=>c.phases.at(-1).name==='baseline' && c.phases.at(-1).worlds===40 && c.phases.at(-1).status==='completed');
  const old40=old.checkpoints.find(c=>c.phases.at(-1).name==='baseline' && c.phases.at(-1).worlds===40 && c.phases.at(-1).status==='completed');
  assert.deepEqual(baseline40.evaluation.options,old40.evaluation.options);
  assert.equal(baseline40.choice,old40.choice);
  rows.push({ply:pos.ply,call,result,checkpoints,baseline40_matches_previous:true});
  writeFileSync(dir+'/panel640.json',JSON.stringify({wasm_sha256:hash,rows},null,2)+'\n');
  console.log(JSON.stringify({ply:pos.ply,seed,choice:tiles[result.choice],options:result.evaluation.options.map(([t,a,b])=>[tiles[t],`${BigInt(b)-BigInt(a)}/${b}`]),elapsed_us:result.elapsed_us}));
}
