...
| O29 | Field identity and purity: every modeled field action is a pure function of its declared information state and immutable FieldId; cross-field comparison reads no hidden data | discipline + tests | data-flow audit, deterministic replay, hidden-world adversarial tests (accepted L2-A2, from `math/targeted_level2_field_stability_v0.1.md` §20) |
| O30 | First-disagreement localization: before the first field action difference, coupled histories and focal actions are identical; no terminal correction attributed to an unsplit world (L2-T1) | proof + tests | paper proof in parent, stepwise assertions, tiny exact fixtures |
| O31 | Exposure-bound typing: FrozenPolicyExposure, LibraryExposure, RootActionExposureUpper mechanically distinct; only the last feeds optimized root-action screening | type/API audit | negative compile or serialization tests |
| O32 | Root field-stability screening: L2-T2..T4 implemented with valid value bounds and valid exposure upper bounds for every legal action | proof + tests | exact small-fiber parity and theorem replay |
| O33 | Field-correction evidence: fixed-policy correction, pair lift, and direct field-1 comparison use the calculated-evidence processes on complete signed values with a complete risk ledger | proof + tests | exact-rational tests and common-world regression |
| O34 | Split-reach optimization correctness: the information-consistent split-reach solver computes or bounds sup over Π_a of Pr(D_ρ = 1) without strategy fusion; clairvoyant versions labeled, consumed only as upper bounds | proof + audit | exact late-grade oracle and information-flow audit |
| O35 | Mechanism-trace fidelity: every explanation names an actual stored first field split and its downstream correction; motif labels never outrun the trace | discipline + tests | trace replay and explanation snapshot tests |
| O36 | Level-model typing: a level-2 result is explicitly best response to σ₁ — never labeled equilibrium, convergence, or monotone improvement | vocabulary audit | API/UI vocabulary audit |
| O37 | Cycle detection discipline: policy recurrence classified by domain completeness; panel recurrence never promoted to exact cycle; exact local cycle requires a dependency-closed domain | discipline + tests | synthetic cycle fixtures and hash/dependency tests |
| O38 | Targeted completeness: no root action excluded from field-1 optimization unless L2-T4 or a stronger exact argument removes it | proof + audit | all-action admission audit and exact small-fiber parity |
