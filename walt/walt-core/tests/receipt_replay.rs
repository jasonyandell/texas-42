...
use walt_core::receipt::{locate_verify_player, parse_file};
use walt_core::replay::{replay_receipt, HAND_TOTAL_POINTS, TRICKS_PER_HAND};
use walt_core::{Decl, DeclClass, DominoSet, Seat};

fn receipt() -> walt_core::receipt::Receipt {
...
fn receipt_declaration_coverage_is_pip_trump_only() {
    ...
    let pip = |v: u8| Decl::PipTrump(walt_core::Pip::new(v).expect("pip"));
...
fn state_and_voids_before_each_trick_are_consistent() {
    ...
    for hand in &r.hands {
        for trick_no in 1..=TRICKS_PER_HAND {
            let (hands, leader) =
                walt_core::replay::state_before_trick(hand, trick_no).expect("a valid hand");
            ...
            let voids = walt_core::replay::voids_before_trick(hand, trick_no);
        ...
        let (empty, _) =
            walt_core::replay::state_before_trick(hand, TRICKS_PER_HAND + 1).expect("a valid hand");
        for h in empty {
            assert!(h.is_empty());
        }
    }
