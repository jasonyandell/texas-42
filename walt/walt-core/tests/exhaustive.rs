...
use walt_core::rules::{Tier, TrickKey};
use walt_core::{Context, Decl, DeclClass, Domino, DominoSet, Pip, Seat, Trick, NATURAL};
...
fn count_decoration_totals_35_and_a_hand_totals_42() {
    ...
    assert_eq!(
        ...
        walt_core::TOTAL_COUNT_POINTS
    );
    assert_eq!(walt_core::TOTAL_COUNT_POINTS, 35);
    // Seven tricks at one point each, plus the whole count decoration.
    assert_eq!(7 + walt_core::TOTAL_COUNT_POINTS, 42);
    assert_eq!(walt_core::replay::HAND_TOTAL_POINTS, 42);
}
    ...
    for decl in Decl::ALL {
        for q in Context::ALL {
            ...
            for bits in [0b1u32, 0b101, 0b1010101, DominoSet::FULL.bits()] {
                ...
                let legal = walt_core::legal_plays(decl, hand, Some(q));
                ...
                assert_eq!(walt_core::legal_plays(decl, hand, None), hand);
            }
        }
    }
}
...
fn seats_and_teams() {
    ...
    assert_eq!(walt_core::Team::T0.seats(), [Seat::S0, Seat::S2]);
    assert_eq!(walt_core::Team::T1.seats(), [Seat::S1, Seat::S3]);
}
