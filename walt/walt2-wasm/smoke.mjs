...
// actual pkg/walt2.wasm binary via the pkg/walt2.ts wrapper, and assert
// the contract, declaration, and all 28 plays are IDENTICAL to the native
// test's frozen trace. Also prints per-decision wall times — the closest
// desk proxy for phone latency (single-threaded wasm, like the browser).
// Exploratory tooling; not a receipt.
//
//   node walt2-wasm/smoke.mjs        (Node >= 23.6 for .ts type stripping)

import { readFile } from 'node:fs/promises';
import { Walt2 } from './pkg/walt2.ts';

// Frozen native trace (cargo test --release -p walt2-wasm -- --nocapture):
const NATIVE = {
  handNo: 1n,
  bidder: 0,
  bid: 42,
  decl: 9,
  record:
    '0 2 1 1 2 12 3 11 0 25 1 21 2 24 3 26 3 5 0 4 1 3 2 17 3 9 0 6 1 7 2 13 3 10 0 0 1 19 2 15 1 8 2 18 3 14 0 16 2 20 3 23 0 27 1 22',
};
const N_BID = 6, N_PLAY = 4, N1 = 2, N0 = 2;
...
const walt2 = await Walt2.load(
  await readFile(new URL('./pkg/walt2.wasm', import.meta.url)),
);
...
for (let s = 0; s < 4; s++) {
  ...
  if (need > 42) break;
  const r = walt2.bid({ hand: hands[s], need, n: N_BID, n0: N0, seed });
...
const d = walt2.declare({ hand: hands[high.seat], bid: high.bid, n: N_BID, n0: N0, seed });
  ...
  for (let pos = 0; pos < 4; pos++) {
    ...
    if (pos === 0 && trick > 0) {
      for (let cand = 0; cand < 4; cand++) {
        try {
          resp = walt2.play({
            ...
            hand: hands[cand], plays: record,
            n: N_PLAY, n1: N1, n0: N0, seed,
          });
          actor = cand;
          leader = cand;
          break;
      ...
      resp = walt2.play({
        ...
        hand: hands[actor], plays: record,
        n: N_PLAY, n1: N1, n0: N0, seed,
      });
    }
    const ms = performance.now() - p0;
    if (ms > maxMs) { maxMs = ms; maxAt = `trick ${trick + 1} pos ${pos}`; }
    assert(resp.level === 2, 'level-2 marker on every play response');
...
console.log(
  `walt2.wasm smoke OK — contract S${high.seat} bid ${high.bid} decl ${d.decl}; ` +
  `28/28 plays match native; ${ms}ms total, slowest decision ${Math.round(maxMs)}ms (${maxAt})`,
);
