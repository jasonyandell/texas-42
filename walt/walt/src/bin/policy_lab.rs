...
fn run() -> Result<String, String> {
    ...
    if args == ["--help"] {
        return Ok("Usage: policy_lab --seed N --tiles 2..7 --samples 1,2,4,8 --test-worlds N --node-budget N --decl 0..7|9 --mode random-own-hand|fixed-root-hand [--hand seven,ids] [--field hash-legal|l0-8|gym] [--artifact-dir PATH]\nExisting gym root: --request FILE --seed N [--exact-test 1]; request uses the seven own/public line fields. Bid is always 30.".into());
    }
    ...
    for &n in &schedule {
        ...
        for offset in 0..3 {
            let arm = (offset + (seed % 3) as usize + n % 3) % 3;
