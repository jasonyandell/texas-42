...
if ! clean_environment_is_exact; then
    ...
    exec /usr/bin/env -i \
        ...
        TMPDIR=/tmp \
        WALT_M2_CLEAN_ENV=1 \
        /bin/bash -p "$bootstrap_dir/build_metallib.sh" "$@"
fi
...
temporary_root=$(/usr/bin/mktemp -d /tmp/walt-m2-metallib.XXXXXX)
...
build_one() {
    ...
    output="$temporary_root/$run_name/walt_m2.metallib"
    /bin/mkdir -p "$source_dir" "$air_dir"
    /bin/cp "$script_dir/00_u256.metal" "$source_dir/00_u256.metal"
    /bin/cp "$script_dir/01_opening_projector.metal" "$source_dir/01_opening_projector.metal"
    ...
    /usr/bin/xcrun --toolchain "$toolchain" metal \
        ...
        -Wextra \
        -Werror \
        -c "$source_dir/01_opening_projector.metal" \
        -o "$air_dir/01_opening_projector.air"
...
/usr/bin/cmp "$temporary_root/first/walt_m2.metallib" \
    "$temporary_root/second/walt_m2.metallib"
if [ "$mode" = "--replace" ]; then
    /bin/cp "$temporary_root/first/walt_m2.metallib" "$script_dir/walt_m2.metallib"
else
    /usr/bin/cmp "$temporary_root/first/walt_m2.metallib" \
        "$script_dir/walt_m2.metallib"
fi
/usr/bin/shasum -a 256 "$script_dir/walt_m2.metallib"
