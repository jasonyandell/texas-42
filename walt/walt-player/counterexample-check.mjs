...
function wasm(call, stopAfterRound=false) {
  ...
  const instance=new WebAssembly.Instance(module,{walt_host:{now_us:()=>stop ? ticks+=3000000n : 0n,
    checkpoint:(ptr,len)=>{const v=JSON.parse(decoder.decode(new Uint8Array(x.memory.buffer,ptr,len)));
      checkpoints.push(v); if(stopAfterRound && v.counterexample_result?.rounds===1)stop=true;}}});
  x=instance.exports;
...
const output={native,wasm:full.value,interrupted:interrupted.value,declarer,checks:'native/wasm parity; complete-round retention; ordinary mode and declarer isolation'};
