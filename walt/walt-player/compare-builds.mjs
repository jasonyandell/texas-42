...
const rows = [];
const save = () => writeFileSync(output, JSON.stringify({schema:'walt-build-comparison-v1',
...
for (const [index,call] of calls.entries()) {
  ...
  for(const field of ['evaluation','fallback_evaluation']) {
    for(const key of ['choice','legal','options','nodes','pi_calls_by_level','inner_worlds_by_level'])
...
console.log('Complete decisions, exact values, search counters and full partner review agree.');
