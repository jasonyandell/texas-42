...
submitted-at: 2026-08-01T14:28:56.756Z
harvested-at: 2026-08-01T15:10:59.683Z
...
1. **`positive_key_injective` finite reduction:** the closed `decide` checks 56,448 declaration/context/tile-pair cases and is the likeliest resource failure. **Fallback:** split `Declaration`, `SuitContext`, and their embedded pips with `cases`/`fin_cases`, then run `decide` separately on each remaining closed `∀ d e` goal.

2. **Inline dependent bounds for `Fin 14` and `Fin 42`:** `omega` may need slightly more explicit unfolding around `Domino.pipSum` or the let-bound rank. **Fallback:** extract separate Nat lemmas `pipSum_lt_14` and `keyCode_lt_42`, prove each with `omega`, and pass those named lemmas to the subtype constructors.

3. **Definitional unfolding in `winner_maximal`:** `simpa [winner]` may not unfold the private maximizer exactly as expected under the pinned elaborator. **Fallback:** replace its body with `unfold winner; exact le_max4Index (playKey δ t)`.
