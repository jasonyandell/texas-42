...
| `kanban/` | The work queue: one file per task, status = its directory (`backlog/` / `doing/` / `done/`), cards linked by greppable `[[card-id]]` tokens, never by path. walt-era work is assigned here; rob's binding assignments remain `rob/BRIEF*.md`. State after 2026-08-24: unification, wiki overhaul, and math reorg done; the adaptive-sampling intake landed and its §22 build ran through step 8, with all three field-swap slices, the playable controller player, and the world-cap 512 ruling applied ([walt-calculated-evidence](walt-calculated-evidence.md)); in flight/queued: CE step 9 (the level-2 probe as detection layer), [[slice3-deferred-producers]], [[panel-response-audits]], [[gran-anchor-reconstruction]], [[m2-receipt-reearn]]. Since 2026-08-30 the counted-belief/anytime era ran to completion off-board ([walt-counted-belief-era](walt-counted-belief-era.md)); the standing queue after it: unification hygiene, the deeper wiki pass, the adaptive-sampling intake, then the level-2 probe. |
...
- [walt-scheme-fix](walt-scheme-fix.md) — the implemented relational expression language: role queries, exact belief events, counterexamples, and the earlier descriptor research.
...
- [walt-calculated-evidence](walt-calculated-evidence.md) — **the era since 2026-08-24**: anytime-valid adaptive settlement as the new correctness path — the two same-day intakes and their exploratory-fence adjudications (CE-A1..A8, L2-A1..A7), the §22 build through step 8 (shadow instrument, V5 flip repair, E0 calibration), the live-player audit findings, the playable controller player, all three field-swap slices through the Part VI cancellation ladder, and the world-cap 512 ruling. Instrument records below every tier; the live player unchanged.
