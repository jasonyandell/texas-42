...
| [`walt/`](walt.md) | The imperfect-information **seat** (rob stays the exact solver), built math-first on its own frozen exploratory-tier basis (`walt/math/`, v0.4) with a conflict-driven lesson factory. Rules→operators stack complete and pinned against every rescued probe record; the lesson spine survived its falsification test, and the seat-label re-measurement then found real, localized label fragility on the big early-trick fibers (cross-validated). Direction reset 2026-08-10: the goal is the lossless count-free equivariant quotient — the v0.5 math track opens with §12.6A (equivariant controlled lumpability, authored by Jason); factory scaling deferred. **Everything exploratory** — see the [walt hub](walt.md) for the fence. |
