...
Family note, so the wager is not mistaken for a verdict: the relational/constellation key is one member of a family of cross-cutting, individually true representations of the same game — the pip/realization layer (exact and load-bearing wherever witnesses are constructed), the rule-free carrier skeleton (measured *incomparable* to the constellation key: neither refines the other, [idea-retrograde-rank](idea-retrograde-rank.md) §7), and the count-weight-field view of the informal capture. This page bets on the relational chart as the one the played game is native to; it does not claim the others reduce to it.

Field evidence of the native factorization: experienced players, told "we evaluate by the relationships between dominoes, not the pips," respond the way one responds to being told a stop sign is red — obviously. They then immediately speak strategy fluently in the frame (finesse patterns keyed to relational situations). Obvious-to-natives is the signature of a captured ubiquitous language; the mathematical content is that the obvious factorization provably suffices for the exact *suffix* value (C1 — exchange-adjudicated tier, Lean mechanization pending).
