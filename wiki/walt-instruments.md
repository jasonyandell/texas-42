...
| `solver::selection`, `solver::partnership` and the partnership arena (2026-09-06, local experiment) | One fixed/refine/race selection authority shared by real and modeled minds; independent seat-level profiles, voidless/counted-void belief strategies, and root/model sample budgets. The bounded player exposes completed comparisons or explicit external fallbacks. Mirrored bid-30 games run in a shared ten-game pool with per-move checkpoints and independent replay. Native racing L1 matched the archived phone in 64 decision checks and all 35 fallback-free full pairs; refined partner configurations exceeded the fallback gate. These are exploratory checks and sampled estimates, not mechanics proofs or calibrated probabilities. Defaults unchanged. | [`FOUNDATION.md`](../experiments/partnership/FOUNDATION.md), [`524-game battery`](../experiments/partnership/campaigns/foundation-battery/RESULTS.md), `partnership --stream`, `experiments/partnership/{player,match,pool}.py` |
...
| `extractreport` | The anytime proof-state Phase 6 instrument [L2 thread, added 2026-08-31, APS-A9]: the §30 bridge on real roots — per root the proof state gets RefineV1 facts plus σ0 baseline profiles, then the §63 `argmax-extraction-v1` producer (full legal set, lowest-tile-index tie rule) extracts each root action's exact best-response continuation as an executable policy DAG, re-prices it through the unchanged fixed-policy evaluator, and installs it under its content id; the report prints the executable bar's rise, DAG sizes, the post-extraction §33 block, and the §63 residual verdicts (closure / escape / empty-class) for three grammars; findings: h3-t4 Γ 83‰ → 0‰ EXACTLY (the 12,420-state DAG materializes 3-1's optimal continuation and the recommendation switches from 4-4 to 3-1 — Phase 3's gap closed by the §63 machinery on schedule), h8-t5 Γ 282‰ → 10‰ (residue = the winner's δ-tier upper), h4-t6 keeps Γ = 133‰ honestly (RefineV1 settles on cross-action dominance so the winner's upper stays vacuous until a §36 upper fact prices it), the two-source grammar ESCAPES on every h3-t4 action (the residual proves where trick-4 grammar room is real) while every multi-source t5/t6 verdict is empty-class (saturation at ≤ 2-tile focal states — Slice E's "ties free" explained structurally) | `probes/factor_belief/` |
| `openingreport` | The anytime proof-state Phase 8 instrument [L2 thread, added 2026-09-01, APS-A9]: the §65 opening-root iterative run — one append-only proof state at h0-t1 (Z = 399,072,960, contract 30 on threes), declared budget stops p=16/64/256/512, the full §65 panel at every stop (bars, U*, Γ, width debt, F masses, policy cylinders, count-threat cells, itemized risk, work in unmixed units, wall, recommendation, typed verdict); findings: the bar climbs 0→407→594→732‰ and Γ falls 1000→592→405→267‰ with the recommendation migrating 0-0→2-1→6-5, the sampled tier PLATEAUS at p=512 (bar and Γ unchanged while the last vacuous uppers break, U* 1000→999‰) — the remaining Γ is policy gap plus lock looseness, purchasable only by structural work; all 29 opening-root frontier refusals are pure affordability (the cliff, not the stall); the whole state is 56 facts / 10,439 bytes, resumed bytewise; verdict honest UNRESOLVED at ε=1/4 — §65's first target met in its certified-regret form | `probes/factor_belief/` |
| `doomreport` | The doom-census instrument [L2 thread, added 2026-09-01]: counterexample mass as deterministic uppers (`solver/doom.rs` — the §70 structural producer, ∀-fail dual of the §16 hierarchy): full-mode census vs per-world enumeration truth on six enumerable receipt roots, the declared top-8 priority census on all seven h0-t1 leads, singleton God-checks (two hand-built crusher worlds + a stride-512 grid), and the composed §65 panel; findings: the census recovers 809–1000‰ of true doom where doom lives (h12-t6 whole-fiber in one decided read; h8-t5's 5-0 at 28/28), the opening root certifies an honest ZERO, and the God grid shows the zero is real — 0 of 228 grid worlds and both crushers let the world-aware viewer make against σ0, so doom-family uppers (floored at the God make rate) cannot move the h0-t1 999‰ ceilings: the plateau's Γ is the info-consistency price, and the census's working domain is the endgame/in-play middlegame | `probes/factor_belief/` |
| `laydownreport` | The anytime proof-state Phase 7 instrument [L2 thread, added 2026-08-31, APS-A9]: the §16 typed laydown census — Laydown / ForcedMake(+witness) / AdversarialPolicyMake / PolicyCertainMake — on structural fixtures and t6 receipt roots, with walk-node counts; findings: the boss-chain control certifies as a TRUE Laydown in 1.49M nodes / 0.37s (proved by walk, not phrase), the already-made root classifies in 3 decided reads (§17 zero-cost live), the loose-boss counterexample refutes fail-fast in 280 nodes, and h10-t6 is a REAL receipt-root Laydown (all four tiers, witness 2-2) | `probes/factor_belief/` |
| `bellmanreport` | The anytime proof-state Phase 4/5 instrument [L2 thread, added 2026-08-31, APS-A8/A9]: per root × action the §61 F STAIRCASE — every stage's root interval, exact mass, and class census to the action-exact endpoint beside the exact §36 response, plus the stage-1 tail envelope's §7 straddle — and the §62 COVER TABLE — verified movement bound vs the §5 arithmetic envelope, named resources, derived rescue-band upper vs the exact response, per-action verdict; findings: the staircase is a real anytime object (h3-t4 [145,606]‰ → exact in 5–6 monotone stages, width = unresolved mass line by line), decided cells stage for free, h12-t6's cover CERTIFIES V* = 0 for one range walk (gain 0 collapses both actions — failure certified below response-walk cost), h4-t6's range walk beats arithmetic by exactly one point with the 5-5 hazard visible at 134‰, and the §70 caveat is live at rich roots (every resource contested → first-generation covers vacuous; richer structural producers are the declared answer) | `probes/factor_belief/` |
| `frontierreport` | The anytime proof-state Phase 1 instrument [L2 thread, added 2026-08-31, APS-A8/A9]: the first anytime SCHEDULES on real roots — per root × goal (SelectAction, RecommendEpsilonPolicy(0), StrengthenToExact) the frontier starts from the top state and buys work items (baseline profile, §36 exact value, §63 targeted extraction, the §41 macro) under a declared Z/3Z forecast cost model, printing every purchase with its §42 bound and debt trajectory plus the §34/§41 refusal census; findings: goal separation is real money (h10-t6/h3-t5 certify Γ = 0 for 1Z while SelectAction costs 7–10Z), h3-t4 SelectAction settles at 16Z WITHOUT buying any extraction (only the ε-goal pays for the DAG), h4-t6 SelectAction is 5Z (one exact upper excludes 0-0; exact(1-1) never bought), the §41 macro is visibly the only §42-lawful mover of the upper side from the top state, and one honest waste is recorded (under vacuous uppers the bounds cannot rank extractions — h3-t4's ε-goal spent 28Z where uppers-first pays ~15Z; §43: a poor forecast wastes, never weakens) | `probes/factor_belief/` |
...
semantics, the σ0 boundary owned by the profile gates' specimen), and
`walt/walt/tests/solver_extraction.rs` (anytime proof-state Phase 6 —
the extracted argmax DAG re-prices to the extraction optimum through
the UNCHANGED fixed-policy evaluator, both grammar and full-legal
sources at every enumerable root child; the extracted profile is one
realizable profile (conservation + contract projection); the §12
cover identity `M* = max(M^G, D)` with the residual walk's `M*` held
to Slice G's recursion; residual closure AND escape in both
directions with three non-vacuity counters, including the discovered
multi-source saturation at t5/t6; the §20 envelope fence with a
tails-cross specimen — the threshold-wise max is no single policy's
record and what extraction serializes is its own evaluation bin for
bin; and the §30 bridge end to end — `B_exec = B_proof` after the
producer at every root, with the discovered vacuous-winner-upper
residue collapsing to Γ = 0 exactly once §36 upper facts price the
upper side), and `walt/walt/tests/solver_frontier.rs` (anytime
proof-state Phase 1 — goal debts typed with zero exactly at
satisfaction; the §42 law recomputed from the report on every executed
item across every root × goal; SelectAction settles with refusal
honesty (a hand-executed zero-potential refusal moves exactly
nothing); the §41 macro moves the upper side while dominated
extractions are skipped; §43 containment — StrengthenToExact lands
every surviving interval on the independently recomputed exact value
with deterministic schedules and byte-identical serializations; and
budget honesty with §44 resume-equals-uninterrupted),
`walt/walt/tests/solver_residual.rs` (anytime proof-state Phase 4 —
the §61 F staircase brackets the exact §36 response at every stage,
nests across stages, and collapses to it at the action-exact endpoint;
the §23 cellwise-max counterexample is REJECTED live (the fused
per-class sum strictly exceeds the lawful merged optimum of its own
domain on h3-t4); staged fixed-policy tail envelopes bracket, nest,
and collapse; an installed `Fact::Envelope` closes to an executable
lower with the first nonzero §7 contract-sensitive residual on the
recommendation; the §41 census law — `ResidualInterval` potential
nonzero exactly where closure can consume an open interval, zero at
the ε-goal's top-state stall, with the §42 law on hand-executions and
the honest flat-3Z dominance of the exact item; and the §62 range
walk's nesting/support laws), and `walt/walt/tests/solver_covers.rs`
(anytime proof-state Phase 5 — accepted covers never understate the
residual gain and their derived rescue-band uppers never undercut the
exact response; declined covers produce no number; zero verified gain
collapses the residual to the incumbent's point; and a small nonzero
count hazard stays visible in the interval, pinned to the §10/§11
band-mass identity), and `walt/walt/tests/solver_laydown.rs` (anytime
proof-state Phase 7 — the boss-chain control is a true Laydown proved
from the rules; the already-made root classifies at zero tree (§17);
the loose-boss and possible-ruff counterexamples break every universal
tier; the hierarchy implications hold on receipt roots with
PolicyCertainMake coinciding exactly with `viewer_success_mass = Z`;
and the producer installs only deterministic facts — §64's
no-sampled-route law — closing immediately on a certified state), and
`walt/walt/tests/solver_opening.rs` (anytime proof-state Phase 8 — the
§25 top state at the zero-budget stop with the §67.4 bytewise round
trip; the sampled ladder narrows monotonically with an exact derived
risk ledger, distinct scopes, and an idempotent presence guard; §67.5
resume ≡ uninterrupted refinement, panel for panel and byte for byte;
the opening root's affordability cliff buys nothing, installs nothing,
and never manufactures a winner (§66.14); and the same ladder settles
enumerable roots exactly under an ample frontier budget with
mass-conserving census coordinates), and
`walt/walt/tests/solver_doom.rs` (the doom census — an already-set
root dooms its whole fiber in one decided read (§17-dual zero-cost);
the loose-boss census meets the exact recursion exactly, phantom
escapes defeated by tile conservation; census uppers never undercut
`response_success_mass` on receipt roots; starved budgets refuse
honestly and deterministically; the producer installs deterministic
uppers idempotently through closure and the byte round trip; the
per-world enumeration equals the census on the fixture, dominates it
on receipt roots, and stays sound; and the priority census is a sound
partial harvest with a populated skip ledger).
