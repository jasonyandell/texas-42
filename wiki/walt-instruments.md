...
`c00717d1`, 2026-09-07) with its purpose, exact invocation, record path, gate
...
- **The σ1-repair (`161b0195`, 2026-09-02, #83)** made `solver::sample_belief`
  ...
  *historical*). Gates R1–R6 in `tests/solver_sigma1_repair.rs`: the before-side
...
choices; an earlier revision of this page cited `6e00528`, which does not touch
`shadow.rs`). The committed `probes/shadow/receipt.jsonl` and `driven.jsonl`
...
| σ1-repair (no binary; `161b0195`, 2026-09-02) | `solver::sample_belief` — the one sampler authority | `tests/solver_sigma1_repair.rs` (R1–R6; fixture `tests/data/sigma1_before_v1.txt`) | `walt/briefs/BRIEF-SIGMA1-REPAIR.md`, `MB0-COLLISION-NOTES.md` | [walt-focal-horizon-era §2](walt-focal-horizon-era.md) |
...
| Binary | Purpose | Invocation | Runtime and output | Status | The sentence it earned |
| --- | --- | --- | --- | --- | --- |
| `rootinterval` | Slice A (§44): on six affordable receipt roots (h4-t6, h10-t6, h5-t6, h12-t6, h8-t5, h3-t5) per legal action the exact Q_a beside the δ-valid root interval — the pmake empirical-max upper (epoch 0) over a frozen `PinnedThenLevel1[2,2]` lower witness (epoch 1, provenance FIXED), δ = 1/20 per endpoint — with shortfall Q − L and excess U − Q, survivor-set evolution by prefix, worlds-to-singleton, the typed decision (DeltaRootWinner / DeltaRootSet / UnresolvedRootSet), wall per phase | `rootinterval run <out.txt> [prefix=16]` | ≈ 0.2 s; **measured 2026-09-13: 0.18 s, identical to run1** | instrument (2026-08-30, #61) | h4-t6 settles `DeltaRootWinner{1-1; bar=7/10}` after 8 sampled worlds (the exact optimum, 13/15 vs 1/3); the four exact ties (h10-t6, h5-t6, h12-t6, h3-t5) come out `UnresolvedRootSet`, never a forced winner; all 14 rows satisfy L ≤ Q ≤ U (gated) |
...
| `partnership_gym` | `walt::gym` (`GymField`: L1 partner at 40/8 voidless, L0-8 opponents, seed 420600, lowest ties) | `tests/gym.rs` (8) + 25 Python tests (`experiments/partnership/test_gym*.py`) | `walt/gym/{mining.json, scenarios/, collections/, specs/, benchmarks/, RESULTS.md}` | walt-gym §1–§8 |
...
| `tiltaudit` | The race is faster per lead and the arena is a dead heat | `walt/TILT-AUDIT.md`; `probes/tilt_arena_2026-08-19.log` | none | The race loses on decision cost in saturation-heavy self-play; Phase E vacuous (deterministic field) |
...
| `fieldswap_cancel` | Λ = 31/1200 asserted; first `FieldDecisionChanged` and first `Dominated` in the wild | `probes/fieldswap_cancel/README.md` | `solver_fieldswap_cancel.rs` (ladder inequalities, containment, winner stability) | Directional rungs ≈ 2.3× tighter than E4 and still no prune |
...
| `rootinterval` | L ≤ Q ≤ U on all 14 rows; h4-t6 winner after 8 worlds | `probes/root_interval/run1.txt`; reproduced 2026-09-13 | `solver_root_interval.rs` (incl. the 11/128 mirror sweep) | Exact ties come out `UnresolvedRootSet` — the instrument refuses to force a winner |
...
| `partnership` (batteries) | L2 Partner vs L1 14/14/72 (50.0%) on 100 shared deals; native `l1-race` reproduces the phone in 64 checks and 35 pairs | `experiments/partnership/campaigns/{foundation-battery,default-partner-battery}/RESULTS.md` | `solver_partnership.rs` (12), `solver_selection.rs` (7); independent replay by `verify_campaign.py` | **No strength gain established anywhere**; native fixed L1 is behind the phone (8/15/77) because the phone races |
...
| --- | --- | --- |
| `scheme` partner-count on h0-t6-S0 | < 10 ms (work 1084) | measured 2026-09-13 |
| `rootinterval run` (prefix 16) | 0.17–0.18 s | run1; measured 2026-09-13: 0.18 s |
...
| `proofreport` | 14.1 s (run1); 7.35 s measured 2026-09-13 | `proofreport_run1.txt` |
...
| The gate (`walt/ci/check.sh`) | 230 s wall, 121 test binaries (CI1); 308 s recorded with FH3's anchors gate; ≥ 18 GB RSS at the anchors suite | `walt/briefs/CI1-REPORT.md`; `walt/LOG.md` (as read by the survey; not re-run here) |

**Newcomer first-run order** — each step run 2026-09-13 from the worktree root
with the release binaries under `walt/target/release/` and `timeout 60`; walls
are `/usr/bin/time -p` real seconds on this machine:

1. `scheme --query walt/scheme/examples/partner-count.scheme --hand 0 --trick 6 --seat 0` — real 0.00 s; prints the coordinate (viewer S0 hand {2-0 4-2}, 6 support worlds), `event-probability: 1/3`, `answer-presence: [count-tile=4-1] probability=1/3`, `work: 1084`. Teaches the coordinate/belief vocabulary.
2. `rootinterval run /tmp/ri.txt` — real 0.18 s; six roots, exact Q beside [L, U] per action; `h4-t6 … decision: DeltaRootWinner{action=1-1;bar=7/10}`, `worlds-to-singleton: 8`; ties typed `UnresolvedRootSet`. Identical to `run1.txt`.
...
8. `partnership_gym --inspect < request` (a seven-line request: `decl 5 / bid 30 / bidder 1 / seat 1 / hand 0 1 2 3 4 5 6 / plays / seed 1`) — real 0.00 s; the `partnership-gym-v1` header with `worlds 399072960`; then `python3 experiments/partnership/gym.py show advantage-01` — real 0.03 s; the exercise with its answer key (`5-0: 30/36 success optimal`, `5-3: 8/36`) and the examiner-only witness.
...
- `walt/ci/run_test_binaries.py`'s `HEAVY_FIRST` names a nonexistent
  ...
  repaired.
- `walt/probes/factor_belief/README.md` line 531 mislabels bellmanreport's
  [145,606]‰ start as action 3-1 (it is 4-1's); the record wins and this page
  says so, the README is not edited here.
