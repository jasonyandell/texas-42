...
**Landings on `main` 2026-09-13 → 09-15 (Sunshine), curated 2026-09-20.** Four paragraphs stood here under a "not yet curated" banner (appended at `58e15cd1`, `0727103e`, `b11aa189`, `a4c2c20d`, `98648370`). They are folded in: the language-level facts are [§10.4](#104-sunshine-2026-09-13--09-15-the-count-offer-query-as-a-live-gate-and-recipes-with-a-selectable-continuation); the experiments are owned elsewhere — the continuation-selectable recipes and `sunshine-recipes-v1` → [walt-gym §11.1](walt-gym.md#111-continuation-selectable-recipes-7abf2aee-validated-b11aa189) (instrument) and [walt-partnership-program §11.4](walt-partnership-program.md#114-the-recipes-campaign-default-l1-under-its-own-continuation-7abf2aee-b11aa189-2026-09-13) (what L1 scored); the bounded live continuation follow-up `l1-partner-rollout` and `sunshine-rollout-v1` → [walt-partnership-program §11.3](walt-partnership-program.md#113-the-partner-rollout-l1-partner-rollout-a4c2c20d-2026-09-13); the sunshine discussion notes → [§11.1](walt-partnership-program.md#111-the-direction-a-reasonable-fairly-quick-partner-aware-player); the future Sunshine harness (2026-09-15) → [§11.7](walt-partnership-program.md#117-the-sunshine-harness-workshop-goals-recorded-not-implemented-98648370-2026-09-15); the first sunshine experiment `l1-partner-count-review` and `sunshine-partner-count-v1` → [§11.2](walt-partnership-program.md#112-the-bounded-partnership-review-l1-partner-count-review-58e15cd1-2026-09-13); the replay of the exact gym misses under deployed L1 → [walt-gym §11.2](walt-gym.md#112-the-targeted-replay-teacher-labels-under-deployed-l1-0727103e). Every number those paragraphs carried is on one of those sections with its record path.
...
### 10.4 Sunshine (2026-09-13 → 09-15): the count-offer query as a live gate, and recipes with a selectable continuation

The experiments are owned by [walt-gym §11](walt-gym.md#11-sunshine-2026-09-13-the-gym-under-deployed-continuations-and-the-live-move-intake) and [walt-partnership-program §11](walt-partnership-program.md#11-sunshine-2026-09-13--09-15-the-partner-aware-live-player); this subsection keeps what is the language's. Curated 2026-09-20 from the four paragraphs that stood at the top of this page under a "not yet curated" banner from 2026-09-13/15. **Sorting (curator, 2026-09-20): adds to an area** — the language itself did not change (`walt/walt/src/scheme/` and `walt/scheme/` are untouched between `c00717d1` and `afd46420`; the module root `policy_search.rs` gained 35 lines, and `policy_search/` two new files — `partner_review.rs` (97 lines, `58e15cd1`) and `partner_rollout.rs` (373 lines, `a4c2c20d`) — with `request.rs` +28/−9; corrected 2026-09-21 from "`policy_search.rs` gained 35 lines for the rollout module", which named only the root file — [walt-architecture §3.9](walt-architecture.md#39-the-2026-09-14--09-20-changes-to-rules-clock-gym-and-policy_search)); what changed is where its expressions are evaluated and by whom.

- **The same Scheme recipe interface now evaluates under named deployed players as well as the original fixed teacher.** `walt/gym/SPECIFICATIONS.md` (`7abf2aee`, 2026-09-13): a specification's `evaluation.contract` is either the teacher (`partnership-gym-v1`, an optimal lawful focal continuation against the fixed field) or `deployed-gym-v1` with named `focal` / `partner` / `opponents` presets from `players.json`. Scheme matching and action valuation have separate cache identities: "Editing Scheme, its presence threshold, the source corpus or domain reuses applicable values for identical own/public requests. Only newly matched coordinates need new values." Support coverage is explicit in every key — `census` (exact for the frozen continuation) or `sample-without-replacement` (an estimate; such recipes refuse `certain=true`). The language gained no solver dependency and no value predicate at this landing either: the continuation is a property of the evaluation stage, never of the query. The [L1 count-offer validation](../experiments/partnership/campaigns/sunshine-recipes-v1/RESULTS.md) regenerates helpful and harmful examples and retains model disagreements (the compare files); "live player changes are measured separately" — on the partnership page.
- **Changing the question without changing the values, measured.** [`walt/gym/queries/offer-five.scheme`](../walt/gym/queries/offer-five.scheme) (`b11aa189`) is `offer-count` with its ten-count `case` dropped — one `case` ending `(count action 5) (beats incumbent action led)`. Over the 96 positions valued under both queries: 0 changed action values, 0 changed best-action sets, 9 changed the helpful/harmful/tied relation; 76 → 25 selected exercises, 0 added, 51 removed ([`count-vs-five.md`](../experiments/partnership/campaigns/sunshine-recipes-v1/count-vs-five.md)) — "exactly the intended separation between question and value" (`RESULTS.md`). Each of the four `--set` query/filter variants took about 11–12 s and created 0 new deployed value keys ([walt-gym §11.1](walt-gym.md#111-continuation-selectable-recipes-7abf2aee-validated-b11aa189)).
- **The count-offer expression is now a live public gate in two optional presets** — `l1-partner-count-review` (`58e15cd1`) and `l1-partner-rollout` (`a4c2c20d`), both 2026-09-13, neither a default. A Python own/public detector "specializes the maintained `offer-count.scheme`" for cheap routing; the native worker evaluates the actual expression and "must agree before a result is accepted" (`sunshine-partner-count-v1/PROTOCOL.md`; `PARTNER-ROLLOUT.md`: "the native worker checks that detector against the actual Scheme expression before changing a move"). The Viewer/World split of §5 is what makes the gate deployable: every relation in `offer-count` is public or own-hand, so the expression the examiner matched is the one the actor evaluates, with no hidden-hand input — "An examiner's access to the actual hidden hands is not an input to the player. Existing examiner-only predicates do not automatically become deployable actor predicates" (`SUNSHINE-NOTES.md`). What the gate's *value judgment* is worth is not a language question: of six teacher-labelled missed offers, two held and four reversed under deployed L1 continuation ([walt-gym §11.2](walt-gym.md#112-the-targeted-replay-teacher-labels-under-deployed-l1-0727103e)); the rollout's own census found its sampled decision can point the wrong way at the deadline ([walt-partnership-program §11.3](walt-partnership-program.md#113-the-partner-rollout-l1-partner-rollout-a4c2c20d-2026-09-13)).
- **The Sunshine direction and the Scheme workshop.** The [sunshine notes](../experiments/partnership/SUNSHINE-NOTES.md) (2026-09-13) propose a fast partner-aware player built from fallible guesses, targeted Scheme checks and hindsight learning, with a reusable experimental method (claim, conditions, intervention, prediction and measurement, challenge) for later belief and dynamics studies — proposals, separate from the implemented capabilities and measured results ([walt-partnership-program §11.1](walt-partnership-program.md#111-the-direction-a-reasonable-fairly-quick-partner-aware-player)). The harness added 2026-09-15 (`98648370`; notes §"Sunshine harness: a Scheme workshop"; "**Future product and research direction; not yet implemented.**") is a designed 3D research bench with linked analytics, reproducible Scheme searches, comparative continuations and shared human/agent operations; its central object is a saved, reproducible experiment whose Scheme expression, belief assumptions, named continuation players, seeds and budgets connect every view; the first workflow to design is "Question → editable Scheme specification → matching positions → comparative replay → saved gym exercise"; "A rich interface does not imply that opening searches or arbitrary Scheme questions become cheap." Recorded for later; the immediate focus returned to the playable game ([walt-partnership-program §11.7](walt-partnership-program.md#117-the-sunshine-harness-workshop-goals-recorded-not-implemented-98648370-2026-09-15)).
