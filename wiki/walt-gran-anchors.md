...
(Added 2026-09-20, cycle 1: the move `backlog/` → `doing/` is commit `6a319216` (2026-09-13, "wiki-book: round 1 — instruments catalog, architecture chapter, GPU side-track chapter, ideas/meta hygiene + kanban, open-questions/FINDINGS finish"), the last commit to touch the card — `git log -1 --format=%h -- kanban/doing/gran-anchor-reconstruction.md`, run 2026-09-21; the card's two open items are unchanged at `afd46420`, and no landing 2026-09-13 → 09-20 touched `walt/probes/gran/` or the two unmerged branches.)
