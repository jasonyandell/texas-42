...
| CFG-03, AUC-05/05A (mark ceiling min(cap,5); auction census) | [rules-profile](rules-profile.md) | PA-B01/B02 (auction state, legality, deterministic transition) proved; PA-B03 (reachable mark ceiling) proved at priority 1 (`Auction.lean`). **Open**: PA-B04, the exact auction history counts for caps 1..7 (REFLECT, priority 3) — the census 2380…3214 rests on the verifiers and rob `verify_objective`. |
