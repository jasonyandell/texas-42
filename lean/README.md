...
Added to this file without altering the layout above; it qualifies the
closing line and records what the layout omits.

- **Receipt files in the layout.** Two committed `#print axioms` transcripts
  sit beside the sources: `trick1_metal_foundation_axioms_v1.txt` (8 entries,
  the receipt for `Texas42/Trick1MetalFoundation.lean`; regenerated by
  `lake env lean Texas42/Trick1MetalFoundation.lean` and byte-diffed by
  `walt/ci/check.sh` on every run and by `walt/ci/check_m2_metal.sh`, the
  elevated freeze-56 Metal gate, when that gate is run) and
  `trick1_perfect_recall_net_axioms_v1.txt` (94 entries, committed in
  97ce321a; consumed by no script; lists 94 of the facade's 103
  `#print axioms` names). Both are pinned by digest in
  `walt/math/gpu_native_trick1_m0_m2_sources_v2.sha256`, so regenerating
  either is a freeze event (FZ-A5).
- **The `Trick1PerfectRecallNet` tree: statements committed; build and audit
  pending.** The M3 bullets above paraphrase the obligations of
  `walt/GPU-NATIVE-TRICK1-M3.md` §12 as stated in the sources. The tree entered
  in commit 97ce321a (2026-08-17, "WIP: M3 perfect-recall net scaffolding
  (mid-flight, does not build)"), no build artifact for it exists in any
  `.lake` on the development machine, no gate builds it, and its receipt is
  short by nine names. "The facade audits every new theorem with
  `#print axioms`" describes what the facade *asks for*, not a recorded
  result. The closing line above ("All theorems depend only on the standard
  axioms …") is therefore verified for the sixteen modules of the priority-0
  era (commit receipts, oleans of 2026-08-03) and the two gated trick-1 modules
  (CI-diffed receipt), and pending for the M3 tree until
  `lake build Texas42.Trick1PerfectRecallNet` succeeds and a receipt with all
  103 names is regenerated. A grep for `sorry`/`native_decide`/`axiom` across
  all 27 files is clean, which is necessary, not sufficient.
- **PROOFS.md rule 2 has a second form.** Rule 2 says to record the
  `#print axioms` receipt in the commit message; every priority-0 commit did.
  Since 2026-08-17 the receipt may instead be a committed file regenerated
  through Lean and byte-diffed by CI, as for the M2 module — the two trick-1
  commits (3b4c6d60, 813d5e81) carry no receipt text in their bodies and the
  file is their receipt. Never hand-edit a receipt file.
- Context, counts and the open queue: `wiki/lean.md` (the chapter),
  `wiki/lean-row-index.md`, `wiki/proof-assistant-plan.md`.
