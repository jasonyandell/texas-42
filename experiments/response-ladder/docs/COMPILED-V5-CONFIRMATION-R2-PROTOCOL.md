...
Use exactly two independent panels, `compiled-v5-confirm-c-20260921` and
`compiled-v5-confirm-d-20260921`. Each contains 16 independent physical deals
...
and fixture hashes live in `results/compiled-v5-confirmation-r2/protocol.json`.
...
## Why this replacement was frozen

The original panel A completed 77 games without player failures. Its second
cycle was refused in preflight because the manually retyped candidate command
contained a nonexistent C1 path. No candidate worker or game started in that
invocation; its return code was 1 after 0.048s. The original partial run and
failed invocation are retained as an incomplete setup failure under the
original strict technical gate. Panel B never started. Neither panel is
promoted or silently relabeled, and no outcome was used to choose a replacement.

The candidate, configuration, sample size, statistical rule and latency gate
are unchanged. Replacement C/D namespaces were frozen before their first game
and checked against all original A/B deals, including unplayed ones. A runner
now derives every cycle's command directly from the frozen identities and
configuration; it never retypes actor paths and refuses continuation after a
failed child invocation. All original evidence remains excluded from these
new confidence calculations and available for audit.
