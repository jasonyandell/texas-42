...
def tasks(path,spec):
    return [(path,seed,arm) for seed in range(spec['start'],spec['start']+spec['count'])
            if not (path/'results'/f'{seed}.json').exists()
            for arm in c.ARMS if not (path/'seeds'/str(seed)/arm/'result.json').exists()]
...
def failed_attempts(folder):
    return sum(c.read(p).get('status')=='failed' for p in (folder/'pool-attempts').glob('*.json'))


def commit_ready(path,spec):
    """Only publish a contiguous seed prefix, regardless of completion order."""
    added=[]
    if (path/'STOP.json').exists(): return added
    for seed in range(spec['start'],spec['start']+spec['count']):
        if (path/'results'/f'{seed}.json').exists(): continue
        result=c.commit_seed(path,spec,seed)
        if result is None: break
        added.append(seed)
        summary=c.summarize(path,spec)
        print(json.dumps({'path':str(path),'seed':seed,'paired':result['paired'],'completed':summary['completed_seeds']}),flush=True)
...
def publish(path,spec,workers,active,token):
    s=c.status(path)
    jobs=[{'seed':seed,'arm':arm,'plies':len(c.read(path/'seeds'/str(seed)/arm/'checkpoint.json',{'decisions':[]})['decisions'])}
          for p,seed,arm in active if p==path]
    s['execution']={'mode':'shared-pool','global_worker_limit':workers,'native_threads_per_game':spec['threads'],'session':token,'active_games':jobs}
    ...
    if jobs: body+='\nActive games: '+', '.join(f"{j['seed']} {j['arm']} {j['plies']}/28" for j in jobs)+'.\n'
...
def advance(paths,workers=10,seconds=260,retries=2):
    ...
    session={'id':token,'started_utc':c.now(),'status':'running','workers':workers,
             'seconds':seconds,'retries':retries,'paths':[str(p) for p in paths],
             'scheduler_sha256':hashlib.sha256(Path(__file__).read_bytes()).hexdigest(),
             'committed':{str(p):[] for p in paths},'attempts':0,'peak_active_games':0}
    began=time.monotonic()
    active={}
    queue=interleave([tasks(p,specs[p]) for p in paths if not (p/'STOP.json').exists()])
    old_handlers={}
    for sig in (signal.SIGINT,signal.SIGTERM):
        old_handlers[sig]=signal.signal(sig,lambda *_:[c.stop(p,'user-interrupt') for p in paths])
    def save_session():
        for path in sessions.values(): c.atomic(path,session)
    def yield_all():
        for path in yields.values():
            if not path.exists(): c.atomic(path,'pause pool slice\n')
    try:
        save_session()
        for p in paths: session['committed'][str(p)]+=commit_ready(p,specs[p])
        last_status=0
        while queue or active:
            ...
            for task,entry in list(active.items()):
                ...
                record.update(returncode=rc,finished_utc=c.now(),end_ply=len(c.read(folder/'checkpoint.json',{'decisions':[]})['decisions']))
                okay=rc==0 and (folder/'result.json').exists()
                interrupted=rc==75 or paused or (p/'STOP.json').exists()
                record['status']='completed' if okay else 'interrupted' if interrupted else 'failed'
            ...
            while queue and len(active)<workers and not paused:
                task=queue.popleft()
                p,seed,arm=task
                if (p/'STOP.json').exists(): continue
                folder=p/'seeds'/str(seed)/arm
                ...
                record={'session':token,'seed':seed,'arm':arm,'workers':workers,
                        'native_threads':specs[p]['threads'],'status':'running','started_utc':c.now(),
                        'start_ply':len(c.read(folder/'checkpoint.json',{'decisions':[]})['decisions'])}
                c.atomic(receipt,record)
                log=(folder/(attempt+'.log')).open('w')
                cmd=[sys.executable,str(c.HERE/'campaign.py'),'arm',str(p),'--seed',str(seed),'--arm',arm,'--yield-path',str(yields[p])]
                try:
                    proc=subprocess.Popen(cmd,stdout=log,stderr=subprocess.STDOUT,env=os.environ.copy())
                except OSError as error:
                    log.close()
                    record.update(status='failed',error=str(error),finished_utc=c.now())
                    c.atomic(receipt,record)
                    queue.appendleft(task)
                    continue
                active[task]=(proc,log,record,receipt)
                session['attempts']+=1
                session['peak_active_games']=max(session['peak_active_games'],len(active))
                save_session()
            if time.monotonic()-last_status>=2:
                for p in paths: publish(p,specs[p],workers,active,token)
                last_status=time.monotonic()
            if not active and (paused or not queue): break
            time.sleep(.1)
        session['status']='slice-complete'
    except BaseException as error:
        session['status']='error'
        session['error']=str(error)
        for p in paths: c.stop(p,'pool-error:'+str(error))
        raise
    finally:
        yield_all()
        ...
        for proc,log,record,receipt in active.values():
            try: rc=proc.wait(timeout=max(.01,cleanup_end-time.monotonic()))
            ...
            log.close()
        for p in paths:
            if session['status']!='error': session['committed'][str(p)]+=commit_ready(p,specs[p])
