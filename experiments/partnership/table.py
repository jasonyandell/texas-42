...
from rules import TILES, legal_tiles, trick_points, winner
...
def main():
    ...
    p.add_argument(
        ...
        help="0..6 pip trump; 7 doubles; 9 no trump",
    )
    ...
    rng = (
        random.Random(args.deal_seed)
        if args.deal_seed is not None
    ...
    print(
        "You declare "
        + str(args.bid)
        + ". Each computer move has a 14-second allowance.\n"
    )
    ...
    for ti in range(7):
        ...
        for pos in range(4):
            ...
            if seat == 0:
                print(
                    "Your hand: "
                    + "  ".join(tile_name(t) for t in sorted(remaining[0]))
                )
                print(
                    "Legal: "
                    + "  ".join(
                        str(i + 1) + ":" + tile_name(t) for i, t in enumerate(legal)
                    )
                )
                tile = None
                while tile is None:
                ...
                response = decide(
                    ...
                    "partner" if seat == 2 else "phone",
                    n1=args.n1,
                    inner_belief=args.inner_belief if seat == 2 else "voidless",
                    selection=args.selection if seat == 2 else "fixed",
                    modeled_selection=args.modeled_selection if seat == 2 else "fixed",
                )
    ...
    print(
        "You made it!"
        if points[0] >= args.bid
        else "Set. You needed " + str(args.bid) + "."
    )
