...
Modes partner/baseline/all-l1 share the native selectable evaluator; phone uses
...
def decide(
    ...
    if review not in ("off", "partner-count") or (review != "off" and mode != "baseline"):
    ...
    if mode in ("baseline", "phone") and modeled_selection != "fixed":
        raise ValueError("this profile has no modeled L1 mind")
    if mode == "phone" and (selection != "fixed" or modeled_selection != "fixed"):
        raise ValueError("phone selection is frozen in its artifact")
    ...
    if (
        any(type(x) is not int for x in (n, n0, n1, budget_ms))
        or not 1 <= n <= 640
        or not 1 <= n0 <= 64
        or not 1 <= n1 <= 64
    ):
    ...
    native_state, status = send([str(BINARY)], native_text(req, "status"), allowance)
    ...
    if len(legal) > 1:
        ...
        fallback_evaluation, status = send(
            [str(BINARY)],
            native_text(
                req,
                "baseline",
        ...
        if mode == "phone":
            text = json.dumps(
                {**req, "seed": str(req["seed"]), "n": n, "n0": n0, "race": True}
            )
            ...
            command = [str(BINARY)]
        evaluation, status = send(command, text, allowance)
        primary_choice = None
...
def main():
    ...
    p.add_argument(
        "--mode", choices=["partner", "baseline", "all-l1", "phone"], default="partner"
    )
    p.add_argument(
    ...
    p.add_argument("--review", choices=["off", "partner-count"], default="off")
    args = p.parse_args()
