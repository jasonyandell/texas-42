...
def verify(path):
    ...
    for r in rows:
        ...
        for a in c.arms_for(spec):
            ...
            ds = snap["decisions"]
            assert len(ds) == 28
            players = c.players_for(spec, a, f["bidder"])
            modes = [p.mode for p in players]
            c.validate_checkpoint(snap, spec, f, seed, a)
            for i, d in enumerate(ds):
                points, lead, _, trick = replay_record(
                    f["hands"], c.record_of(ds[:i]), f["decl"], f["bidder"]
                )
            ...
            points, _, remaining, trick = replay_record(
                f["hands"], c.record_of(ds), f["decl"], f["bidder"]
            )
            ...
            assert result["overbid_points_diagnostic"] == max(
                0, points[f["bidder"] % 2] - 30
            )
            ...
            assert result["max_decision_us"] == max(
                d["response"]["elapsed_us"] for d in ds
            )
            assert result["trick_us"] == [
                sum(d["response"]["elapsed_us"] for d in ds[t : t + 4])
                for t in range(0, 28, 4)
            ]
            assert max(result["trick_us"]) < 60_000_000
        expected = (
            c.match_pair(r["arms"]["declaring"]["made"], r["arms"]["defending"]["made"])
            if "players" in spec
            else c.paired(*(r["arms"][a] for a in c.ARMS))
        )
        assert r["paired"] == expected
    receipt = {
        "campaign": spec["id"],
        "verified_utc": c.now(),
        "seeds": len(rows),
        "games": len(c.arms_for(spec)) * len(rows),
        "decisions": 28 * len(c.arms_for(spec)) * len(rows),
        "status": "passed",
    }
