...
class CampaignTests(unittest.TestCase):
    def test_only_make_ranks_and_seed_unit_is_bounded(self):
        ...
        self.assertEqual(c.paired(*[{"made":False,"points":[n,42-n]} for n in (7,29,1)])["ties"], 2)
    ...
    def test_same_hand_worlds_preserve_information(self):
        ...
        for group in range(10):
            ...
            for f in worlds:
                ...
                self.assertEqual((f["bidder"],f["decl"],f["hands"][f["bidder"]]),
                                 (first["bidder"],first["decl"],first["hands"][first["bidder"]]))
            self.assertGreater(len({tuple(f["hands"][(f["bidder"]+2)%4]) for f in worlds}), 1)
    ...
    def test_atomic_completion_and_identity(self):
        with tempfile.TemporaryDirectory() as tmp:
            ...
            for arm in c.ARMS:
                c.atomic(path/"seeds"/"420601"/arm/"result.json", {
                    "campaign":spec["id"],"seed":420601,"arm":arm,
                    "fixture":c.fixture(spec,420601),"made":False})
    ...
    def test_early_stopping_and_world_cluster_exclusion(self):
        s={"nonforced_decisions":100,"fallbacks":0,"fresh_seeds":10,
           "downside_e_value":{"numerator":"6561","denominator":"256"},
           "fresh_reference_makes":0,"fresh_paired":{"wins":0,"losses":8,"ties":12}}
        self.assertEqual(c.early_reason(s,{"panel":"random"}),"downside-e-value-at-least-20")
        self.assertIsNone(c.early_reason(s,{"panel":"worlds"}))
        s["fresh_seeds"]=9
        self.assertIsNone(c.early_reason(s,{"panel":"random"}))
        s["fallbacks"]=6
        self.assertEqual(c.early_reason(s,{"panel":"random"}),"fallback-rate-exceeds-five-percent")
