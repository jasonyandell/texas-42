...
SOURCE_FILES = ["campaign.py", "player.py", "rules.py", "phone.mjs", "reference/phone/walt.ts", "reference/phone/walt.wasm"]
...
def identities():
    return {**{p: hashlib.sha256((HERE/p).read_bytes()).hexdigest() for p in SOURCE_FILES},
...
def initialize(path, start=420600, count=100, panel="random", threads=4, workers=3):
    ...
    spec = {"schema": "partnership-campaign-v1", "start": start, "count": count,
            ...
            "identities": identities(), "created_utc": now(),
...
def load(path, verify=True):
    ...
    if verify and spec["identities"] != identities():
        raise ValueError("source/binary identity changed; refuse silent resume under a different player")
    return spec


def trump_rank(hand, trump):
    called = [t for t in hand if trump in TILES[t]]
    return (len(called), int((trump, trump) in [TILES[t] for t in called]),
            sum(TILES[t][0] == TILES[t][1] for t in hand if t not in called), -trump)
...
def fixture(spec, seed):
    if spec["panel"] == "random":
        ...
        bidder, decl = max(((s, d) for s in range(4) for d in range(7)),
...
def arm_run(path, seed, arm, yield_path):
    ...
    while len(decisions) < 28:
        ...
        points, lead, remaining, trick = replay_record(f["hands"], record, f["decl"], f["bidder"])
        ...
        assert response["phases"][0]["status"] == "completed", "independent rules disagreement"
        for phase in response["phases"]:
            assert not phase["status"].startswith("rejected:"), "invalid worker response"
    ...
    trick_us = [sum(d["response"]["elapsed_us"] for d in decisions[t:t+4]) for t in range(0,28,4)]
    assert max(trick_us) < 60_000_000, "four-play trick budget exceeded"
    counts = Counter(d["response"]["route"] for d in decisions)
    result = {"campaign": spec["id"], "seed": seed, "arm": arm, "fixture": f,
              "modes": modes, "points": points, "made": points[f["bidder"]%2] >= 30,
              "overbid_points_diagnostic": max(0, points[f["bidder"]%2]-30),
              "decisions": 28, "routes": dict(counts), "trick_us": trick_us,
...
def paired(phone, declaring, defending):
    ...
    return {"declaring_delta": attack, "defending_delta": defense,
            "wins": int(attack>0)+int(defense>0),
            "losses": int(attack<0)+int(defense<0),
            "ties": int(attack==0)+int(defense==0), "seed_delta": attack+defense}


def commit_seed(path, spec, seed):
    arms = {a: read(path/"seeds"/str(seed)/a/"result.json") for a in ARMS}
    if not all(arms.values()):
        return None
    for a, r in arms.items():
        assert (r["campaign"], r["seed"], r["arm"]) == (spec["id"], seed, a)
        assert r["fixture"] == fixture(spec, seed)
    result = {"campaign": spec["id"], "seed": seed, "arms": arms,
              "fresh": seed not in spec["known_development_seeds"],
              "paired": paired(*(arms[a] for a in ARMS)), "committed_utc": now()}
    if spec["panel"] == "worlds":
        for old in complete_results(path, spec):
            if old["arms"]["phone"]["fixture"]["group"] != arms["phone"]["fixture"]["group"]:
                continue
            for a in ARMS:
                if "fallback" not in old["arms"][a]["opening_route"] and "fallback" not in arms[a]["opening_route"]:
                    assert old["arms"][a]["opening_choice"] == arms[a]["opening_choice"], "same-information opening changed across hidden worlds"
...
def summarize(path, spec):
    ...
    def tally(rows):
        return {k: sum(r["paired"][k] for r in rows) for k in ("wins", "losses", "ties")}
    wf, lf = sum(r["paired"]["seed_delta"]>0 for r in fresh), sum(r["paired"]["seed_delta"]<0 for r in fresh)
    ...
    summary = {"campaign": spec["id"], "panel": spec["panel"], "completed_seeds":len(results),
               "target_seeds":spec["count"], "fresh_seeds":len(fresh), "completed_games":len(arms),
               "all_paired":tally(results), "fresh_paired":tally(fresh),
               "downside_e_value": {"numerator":str(e.numerator), "denominator":str(e.denominator)},
               "fresh_reference_makes":sum(r["arms"]["phone"]["made"] for r in fresh),
               "fallbacks":fallback, "nonforced_decisions":nonforced,
               "max_trick_us": max((max(a["trick_us"]) for a in arms), default=0),
               "max_decision_us":max((a["max_decision_us"] for a in arms),default=0),
               "stop":stop, "updated_utc":now()}
    return summary


def early_reason(s, spec):
    if s["nonforced_decisions"] >= 20 and s["fallbacks"]*20 > s["nonforced_decisions"]:
    ...
    if spec["panel"] == "random":
        ...
        if s["fresh_seeds"] >= 20 and s["fresh_reference_makes"] == 0 and s["fresh_paired"]["wins"]+s["fresh_paired"]["losses"] == 0:
...
def status(path, active=None):
    ...
    if active is not None:
        ...
        s["active_plies"]={a:len(read(path/"seeds"/str(active)/a/"checkpoint.json",{"decisions":[]})["decisions"]) for a in ARMS}
    else:
        pending=[seed for seed in range(spec["start"],spec["start"]+spec["count"]) if not (path/"results"/(str(seed)+".json")).exists()]
        if pending:
            s["next_seed"]=pending[0]
    atomic(path/"status.json",s)
    pair=s["fresh_paired"]
    state="STOPPED: "+s["stop"]["reason"] if s["stop"] else "COMPLETE" if s["completed_seeds"]==s["target_seeds"] else "Resumable"
    body=(f"# Texas 42 — {spec['panel']} campaign\n\n{state}\n\n"
    ...
    if active is not None:
        body+=f"Active seed **{active}**: "+", ".join(f"{a} {v}/28" for a,v in s["active_plies"].items())+".\n\n"
...
def advance(path, seconds=260, max_seeds=100):
    if not 1 <= seconds <= 270:
        raise ValueError("advance must be 1..270 seconds; launch beneath 295-second watchdog")
    path=Path(path).resolve()
    spec=load(path)
    ...
    session={"id":token,"started_utc":now(),"command":sys.argv,"seconds":seconds,"status":"running"}
    ...
    try:
        for seed in range(spec["start"],spec["start"]+spec["count"]):
            if (path/"results"/(str(seed)+".json")).exists():
                continue
            if (path/"STOP.json").exists() or time.monotonic()-began>=seconds or completed_now>=max_seeds:
                break
            if commit_seed(path,spec,seed):
                ...
                if reason:
                    ...
                    break
                continue
            pending=[a for a in ARMS if not (path/"seeds"/str(seed)/a/"result.json").exists()]
            seed_start=time.monotonic()
            last_status=0
            while pending or processes:
                elapsed=time.monotonic()-began
                if (elapsed>=seconds or time.monotonic()-seed_start>=spec["seed_cap_seconds"]) and not yield_path.exists():
                ...
                while pending and len(processes)<spec["workers"] and not paused:
                    ...
                    files.append(log)
                    cmd=[sys.executable,str(Path(__file__).resolve()),"arm",str(path),"--seed",str(seed),"--arm",arm,"--yield-path",str(yield_path)]
                    processes[arm]=subprocess.Popen(cmd,stdout=log,stderr=subprocess.STDOUT,
                ...
                for arm,proc in list(processes.items()):
                    ...
                    if result not in (0,75) and not paused:
                        stop(path,"worker-failure:"+str(seed)+":"+arm+":"+str(result))
            ...
            if committed:
                completed_now+=1
                summary=status(path)
                print(json.dumps({"seed":seed,"paired":committed["paired"],"completed":summary["completed_seeds"],"fresh":summary["fresh_paired"]}),flush=True)
                reason=early_reason(summary,spec)
                if reason:
                    stop(path,reason)
            if yield_path.exists() or (path/"STOP.json").exists():
                break
        session["status"]="paused" if (path/"STOP.json").exists() else "slice-complete"
...
def main():
    ...
    p.add_argument("command",choices=("init","advance","status","stop","resume","arm"))
    ...
    if args.command=="init":
        ...
        if stopped.exists():
            atomic(args.path/"control"/(str(uuid.uuid4())+"-resumed.json"),read(stopped))
            stopped.unlink()
        print(json.dumps(status(args.path)))
    else:
        print(json.dumps(status(args.path)))
