...
6. `docs/40_CLAIM_STATUS.md`, claims `ALG-01` through `ALG-24` plus
   `REACH-05`

Do not use another Texas 42 implementation as source code or rule authority.
The files under `verification/` are independent proof receipts for this
package; do not copy them as the implementation.
...
9. contextual `BEATS`;
10. when-led `THREAT`;
11. count points and their sum-five/sum-ten antidiagonal identity;
12. the looped-`K7` / symmetric-square presentation;
13. declaration-local competitive ordinals;
14. all ordered pip-trump unscored mechanics transports;
15. the exact three unscored declaration-mechanics classes; and
16. exact finite verification of the claims assigned below.
...
optimization code. Do not modify any repository-level proof receipt:

- `verification/verify_foundation.py`;
- `verification/verify_minimality_and_reachability.py`; or
...
DeclarationAlgebra.competitive_ordinal(led_suit, id)

PipTrumpTransport
pip_trump_transport(source_pip, target_pip)
PipTrumpTransport.pip_map
PipTrumpTransport.domino_map(id)
PipTrumpTransport.context_map(led_suit)
...
### Looped graph and count antidiagonals

Exhaustively verify:

- the domino universe is exactly the symmetric square of the seven pips;
- equivalently, it is the 28-edge set of complete looped `K7`;
- every natural incidence set is the corresponding closed star; and
- `count_points(d) == pip_sum(d)` exactly for sums 5 and 10 and is zero
  otherwise.
...
### Unscored pip-trump transports and mechanics classes

For every ordered pair `(source, target)` of pips:

1. build the unique map sending `source` to `target` and preserving order on
   the two six-pip complements;
2. transport both endpoints of every domino and every natural led context;
3. verify called and powered membership, effective incidence, led context,
   follow, and every pairwise contextual-order comparison; and
4. verify exact round-trip bijectivity.

Assert all 49 ordered transports succeed. Then compute the structural class
signature from the implemented relation surface and assert exactly three
unscored mechanics classes:

```text
all seven pip trumps
DOUBLES_TRUMP
NO_TRUMP
```

Do not include count decoration or literal numeric rank labels in this
unscored isomorphism. The separate scored classification below remains
narrower.
...
looped K7 / symmetric-square identity: PASS
unscored ordered pip-trump transports: 49
unscored declaration mechanics classes: 3
...
- copy any repository-level verification program;
- hard-code 28-by-declaration result tables or declaration-class tables as
  source data;
