...
```text
dominoes == SymmetricSquare(PIPS)
dominoes == edgesOfCompleteLoopedGraph(PIPS)
naturalIncidence(p) == closedStar(p)
countPoints(d) == pipSum(d) if pipSum(d) in {5, 10} else 0
```

The explicit five-tile count list may be exposed for display, but the
antidiagonal formula is the normative executable identity.
...
Additional exact operations:

```text
competitiveOrdinal(ledSuit, domino) -> integer in [0, 13]
    # zero exactly for a slough; positive ordinals preserve trick-key order

PipTrumpTransport(sourcePip, targetPip):
    pipMap: source pip -> target pip and order-preserving on complements
    dominoMap(domino) -> DominoId
    contextMap(ledSuit) -> LedSuit
```

`PipTrumpTransport` must preserve the unscored mechanics surface exactly:
called/powered membership, effective suits, led context, follow, and every
pairwise contextual comparison. All 49 ordered pip-trump transports are valid.
The nine declarations therefore expose three unscored mechanics classes:
...
Count decoration remains separate. Only identity and `2 <-> 3` survive the
stronger scored count-preserving pip-transport contract.
...
ReachableContractedPlayState:
    opaque validated ContractedPlayState produced only by
    beginContractedPlay, applyPlay, or exact projection from a validated
    FullLocationState plus its legal-history witness
...
type. Structural validation alone may not mint a reachability certificate.
Every reachable state carries an opaque certificate tied to one exact
`ContractedHandOrigin`; `applyPlay` preserves that certificate. The certificate
is proof/provenance metadata, not an additional rule variable in physical
transition.
...
contain seven completed tricks. Raw structural fields without a reachability
certificate are not accepted for settlement.
...
AuctionMechanicalState:
    ...
    initialCells: RuleDerivedCellSystem
        # unseen pool, possible=pool, capacity=7
    phase: AUCTION
...
This reference coordinate is an evidence-rich exact presentation, not a
minimal transition payload. Section 17B defines an alternative reduced
viewer play/support kernel that stores the canonical support normal form and a
folded trick residue, and does not also store raw void provenance or completed-
play attribution as physical state. An implementation may use either semantic
source and derive/cache the other; it must not allow both to diverge.
...
A `RuleDerivedCellSystem` in a mechanical projection additionally satisfies:
...
```text
Determinate:
    ambiguousPool is empty
    every residual capacity is zero

Binary:
    ambiguousPool is nonempty
    1 <= firstActiveResidual < len(ambiguousPool)
    every ambiguous domino can occupy either active seat

Ternary:
    residual0, residual1, residual2 are positive
    residual0 + residual1 + residual2 == len(ambiguousPool)
    every exclusion key lies in ambiguousPool
    each domino excludes at most one seat
    for each seat s:
        len(ambiguousPool) - exclusionCount[s] >= residual[s] + 1
```
...
approximation to Hall feasibility. Every stored ternary exclusion is essential:
...
`FeasibleSupportNormalForm` and its source cells decode to exactly the same
fiber. Branch-specific exact compiled forms are permitted:
...
all nine declaration algebras, and the initial unconstrained three-seat cell
system. Straight bid legality does not reduce rule support.
...
- hidden capacities;
- unseen pool;
- possible-holder sets and cells.
...
==
set(RemainderFiber(projectContractedInformation(infoState).cells))
```
...
require d in preState.cells.unseenPool
require d in preState.cells[actor].possible
...
remove d from unseenPool
remove d from every hidden possible set
decrement actor capacity

if q is not None and not followed:
    add q to publicVoids[actor]
    remove every q-follower from actor possible set
...
## 17A. Minimal exact support transition state

`FeasibleSupportNormalForm` is a closed exact support-transition object when
the public action context is supplied.

```text
HiddenObservation :=
    LEAD(actor, domino)
    | FOLLOW(actor, domino, ledSuit)
    | SLOUGH(actor, domino, ledSuit)

advanceExactSupport(
    support: FeasibleSupportNormalForm,
    declaration: Declaration,
...
```text
decode support to its reduced marginal-holder graph
require domino -> actor is a supported holder edge
if SLOUGH:
    require not follows(domino, ledSuit)
    delete actor edges to every follower of ledSuit
if FOLLOW:
    require follows(domino, ledSuit)
force domino -> actor
remove domino and decrement actor residual capacity
compile the matching-supported core
encode the unique determinate/binary/ternary normal form
reject an empty successor as ImpossibleObservation
reject a malformed or inconsistent typed context as InvalidSupportTransitionContext
```

`LEAD` carries no follower-set deletion. A viewer play uses the identity on
hidden support and is handled by the containing viewer kernel.
...
decode(postSupport)
  == typedRemoval(
         decode(preSupport) restricted to worlds making observation legal
     )

for every surviving domino d:
    marginalHolders(post, d) subseteq marginalHolders(pre, d)

ambiguity phase never rises:
    TERNARY -> TERNARY | BINARY | DETERMINATE
    BINARY  -> BINARY | DETERMINATE
    DETERMINATE -> DETERMINATE
```

An initial Straight hidden support has 63 holder edges. During one hand, every
edge is deleted exactly once; at most 42 deletions occur while the corresponding
tile remains live. The normal form is a support transition state, but it is not
by itself a complete game state: declaration, actor order, and led-context
residue are required inputs.

## 17B. Folded trick and reduced viewer play/support kernel
...
OpenTrickFold:
    ledContextIndex: integer in [0, 6]  # declaration-local leadable context
    currentStrength: integer in [1, 13]
    currentWinner: Seat
    pendingCountUnits: integer in [0, 5]  # multiply by five

TrickResidue := TrickBoundary | OpenTrickFold
```

The fold transition compares a new domino's declaration/context competitive
ordinal with `currentStrength`, updates `currentWinner` only on a strict
increase, and adds its count units. `pendingCountUnits` is bounded by five only
while the trick remains unresolved. On the fourth-play transition, compute the
final count in a widened local value (at most six units, or 30 count points),
emit the trick result, and clear the open fold rather than storing that transient
value in `OpenTrickFold`. On the fourth play it emits:

```text
winner = currentWinner after the update
trickPoints = 1 + 5 * pendingCountUnits after the update
next residue = TrickBoundary(winner)
```

Open-trick remaining-hand capacities must contain exactly two adjacent values.
The low-capacity seats form one nonempty proper clockwise interval. Derive:

```text
leader = unique low seat whose predecessor is high
playsMade = number of low seats
nextActor = leader + playsMade mod 4
```

At a boundary, all four capacities are equal and `nextLeader` is explicit.

```text
UtilityAccumulator[U]:
    transition(trickResult or handBoundary) -> UtilityAccumulator[U]
    terminalValue() -> exact U value when terminal
```

Named exact examples:

```text
AdditivePoints: no accumulated score in continuation state; use transition
                rewards and add any already banked root offset externally
SignedDifferential: same convention with signed transition rewards and any
                    already banked root offset external
PointContract: capped remaining declaring threshold
MarkContract: sweepAlive boolean
MatchUtility: exact marks, shaker, target, and named cross-hand residue
```
...
ReducedViewerKernel[U]:
    declaration: Declaration
    ownRemainingHand: FrozenSet[DominoId]
    hiddenSupport: FeasibleSupportNormalForm
    trick: TrickResidue
    utility: UtilityAccumulator[U]
    fixed contract/viewer-frame parameters supplied by the containing problem
```

This kernel determines viewer legal actions, actor order, exact hidden-action
acceptance and support transition, trick winner/reward, hand termination, and
the selected utility transition. It does not contain the evidence record used
by a history-reading policy or belief model.

Score identity available to validators:

```text
playsInOpenTrick = 0 at boundary, else number of low-capacity seats
completedTricks
  = (28 - len(ownRemainingHand)
        - len(hiddenSupport.unseenPool())
        - playsInOpenTrick) / 4
bankedTotal
  = completedTricks
    + 35
    - count(ownRemainingHand)
    - count(hiddenSupport.unseenPool())
    - pendingCount
```

Both partnership point totals must not be stored when only their sum and one
side are needed. No universal utility-independent score payload is required or
claimed.
...
ReachableMechanicalState:
    opaque certified wrapper around MechanicalState

ReachableSupportNormalForm:
    opaque certified wrapper around FeasibleSupportNormalForm
...
SymbolicSupportTraceWitness:
    viewer: Seat
    viewerInitialHand: FrozenSet[DominoId]
    contract and declaration residue
    initialLeader: Seat
    actor-attributed legal public play prefix
    claimedSupport: FeasibleSupportNormalForm

ReachabilityOuterCertificate:
    declaration: Declaration
...
validateSymbolicTrace(witness: SymbolicSupportTraceWitness)
    -> ReachableSupportNormalForm
```

`validateReachability` replays from a supplied complete deal. The smaller
`validateSymbolicTrace` starts from unrestricted 7-7-7 support on the 21
unseen tiles, checks viewer plays against the known hand, checks hidden plays
by `advanceExactSupport`, advances the public trick residue, and requires exact
equality with `claimedSupport`.

The symbolic validator is sound and complete for public trace realizability:
it accepts exactly when at least one complete deal legally realizes the trace.
A hidden deal is therefore optional certification evidence, not required
semantic payload. Membership of a candidate support in the full reachable
image remains decidable by finite exhaustive trace or deal-witness search, but
that search is not required as an ordinary API.

A reachable value stores no `reachable: bool`. Legal constructors preserve the
invariant inductively. An external value remains `Uncertified` until exact
validation succeeds; after certification, a validator tag or witness may be
erased because it does not refine the semantic state.

`ReachableSupportNormalForm` is not a complete game state. It omits
declaration, actor order, current trick, utility residue, and the realizing
path. It may be passed to `advanceExactSupport` only together with the complete
typed public observation context. Full game transition uses either
`ReachableMechanicalState` or `ReducedViewerKernel`; no API may infer missing
public context from a standalone support identifier.
...
having no legal Straight ancestry. `ReachabilityOuterCertificate.validate()`
...
The upper numbers rank necessary outer certificates and are constructive
...
NativeHand:
    ...
    physicalSupport: MechanicalState | ReducedViewerKernel[SelectedUtility]
    exactSupport: derived RemainderFiber view

Invariant when physicalSupport is MechanicalState:
    exactSupport == RemainderFiber(physicalSupport.cells)

Invariant when physicalSupport is ReducedViewerKernel:
    exactSupport == decode(physicalSupport.hiddenSupport)
```

This object keeps the full 28-node algebra available. The viewer hand is a
marking on the ambient universe, not an isolated graph. `exactSupport` is a
derived semantic view, never a second independent authority: it is decoded
from the rule-derived cells in the evidence-rich presentation or from the
minimal support normal form in the reduced presentation.
...
supportSuccessor(typedObservationContext)
    -> typed successor physicalSupport/exactSupport
```
...
a belief-bearing decision object, not to `NativeHand` alone. There is no
...
```text
ContinuationRecord:
    exact viewer-known observation residue, beyond the selected physical/support
    state, that the selected decision strategy, continuation field, or utility
    can still consult
...
DecisionState:
    physicalSupport:
        MechanicalState
        | ReducedViewerKernel[SelectedUtility]
...
exact strategic state \((K,e,\beta)\) or \((c,e,\beta)\) relative to a fixed
rules profile, continuation model, utility, and allowed decision-strategy
class. Any utility-specific residue is already part of the selected
`physicalSupport` or `continuationRecord`; it must not appear as a second
...
`Domino`, `NativeHand`, `RemainderFiber`, `MechanicalState`, or
`ReducedViewerKernel`.
...
FutureOutputContract:
    currentOutput(state) -> finite exact output
    transitionOutput(state, action, successor) -> finite exact output
    includeLegality: bool

minimizeReachableMachine(
    initialStates,
    actionAlphabet,
    partialDeterministicTransition,
    outputContract
) -> FutureEquivalenceQuotient
```

Two states are equivalent exactly when every finite action word yields the same
legality/output trace. The returned quotient is the unique smallest reachable
deterministic exact machine for that selected output contract, up to
isomorphism. Changing the contract changes the quotient. In particular,
support-aware, physical-only, and utility-specific minima are different
questions.

The explicit `ReducedViewerKernel` is a proved sufficient representation. Do
not label it globally transition-minimal unless comparison with the selected
future-equivalence quotient proves equality.
...
InvalidSupportTransitionContext
DealOutsideSupport
...
- Reachability-certificate equality compares exact lifecycle provenance and is
  distinct from contracted-play physical equality.
...
- Mechanical-state equality compares its explicit fields only.
...
11. looped-`K7`/symmetric-square domino identity and count-antidiagonal
    identity;
12. all 49 ordered unscored pip-trump mechanics transports and the exact three
    unscored declaration classes;
13. count-preserving pip permutations and scoped `2 <-> 3` scored game-order
    isomorphism;
14. the negative witness that when-led threat is not a complete ontology.
