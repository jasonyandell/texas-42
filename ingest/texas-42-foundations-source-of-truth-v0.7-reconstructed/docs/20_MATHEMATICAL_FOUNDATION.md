...
is the edge set of the complete graph on the seven pip vertices with one loop
at every vertex. Mixed dominoes are ordinary edges and doubles are loops. This
looped-\(K_7\) description is not an analogy: it is an isomorphic presentation
of the physical domino universe.
...
**[COROLLARY — proved mathematically]** In the looped-\(K_7\) presentation,
\(\sigma_p\) is exactly the closed star of vertex \(p\): its loop together
with the six ordinary edges incident to \(p\).

**Proof.** Edge incidence in \(\operatorname{Sym}^2(\mathbb P)\) is literal
pip membership. ∎
...
**[DEFINITION]** The Straight count decoration is the two-antidiagonal weight

\[
c(d)=
\begin{cases}
\operatorname{sum}(d),&\operatorname{sum}(d)\in\{5,10\},\\
0,&\text{otherwise.}
\end{cases}
\]

Equivalently, the three sum-five tiles

\[
5\!:\!0,\quad4\!:\!1,\quad3\!:\!2
\]

are worth five, and the two sum-ten tiles

\[
6\!:\!4,\quad5\!:\!5
\]

are worth ten. Thus count is a fixed decoration of two antidiagonals of the
triangular pip lattice; it is not an additional arbitrary lookup relation.

**[THEOREM — proved mathematically]**

\[
\sum_{d\in\mathcal D}c(d)=3\cdot5+2\cdot10=35.
\]

**Proof.** The unordered endpoint pairs with sum five are exactly the three
listed pairs, and those with sum ten are exactly the two listed pairs. Every
other domino has zero count label. ∎
...
**[THEOREM — exhaustive finite verification]** The independent verifier checks
all
...
**[THEOREM — exhaustive finite verification]** An independent verifier checks
...
### 3.10 Three unscored declaration-mechanics classes

The count decoration affects reward but not legal play or trick winner. To
separate those roles, let the **unscored mechanics structure**
\(\mathcal M_\delta\) retain:

- the physical domino and context sorts;
- called and powered membership;
- effective-suit incidence;
- led-context and follow relations; and
- the strict contextual comparison order induced by trick keys;

while omitting \(c\) and omitting the literal numeric labels used to encode
rank. Only order is retained.

For pip trumps \(t,u\in\mathbb P\), define \(f_{t,u}:\mathbb P\to\mathbb P\)
by

1. \(f_{t,u}(t)=u\); and
2. mapping the \(i\)-th smallest member of \(\mathbb P\setminus\{t\}\) to the
   \(i\)-th smallest member of \(\mathbb P\setminus\{u\}\).

Extend \(f_{t,u}\) to domino endpoints and natural contexts, fixing called
context 7.

**[THEOREM — proved mathematically: all pip-trump mechanics are isomorphic]**
For every \(t,u\in\mathbb P\), the transported map is an isomorphism

\[
\mathcal M_t\cong\mathcal M_u.
\]

**Proof.** The endpoint map is a bijection and sends the closed star of
\(t\) to the closed star of \(u\), so called and powered membership commute.
It sends every effective incidence to the corresponding transported
incidence. On an uncalled domino both ends lie outside \(t\), where
\(f_{t,u}\) is order-preserving; therefore `high`, led context, and natural
follow transport. In the powered chain, the trump double maps to the trump
double and remains top, while mixed trump order is the order of the other end,
which \(f_{t,u}\) preserves. In a natural effective suit, the double remains
top and the mixed order is again the preserved order of the other end after
the called edge has been removed. Tier and every strict contextual comparison
therefore commute. ∎

**[THEOREM — proved mathematically: exact three-class classification]** The
nine Straight declarations have exactly three isomorphism classes as unscored
mechanics structures:

1. one class containing all seven pip trumps;
2. doubles-trump; and
3. no-trump.

**Proof.** The preceding theorem gives one pip-trump class. The pair

\[
\left(
|\pi_\delta|,
|\{d\notin\pi_\delta:|\widehat\Sigma_\delta(d)|=1\}|
\right)
\]

is invariant under any isomorphism of the named structure. Its values are

\[
(7,6)\quad\text{for pip trump},\qquad
(7,0)\quad\text{for doubles-trump},\qquad
(0,7)\quad\text{for no-trump}.
\]

The three classes are therefore pairwise nonisomorphic. ∎

Count decoration destroys most pip-trump transports. Restoring \(c\) recovers
the narrower count-preserving classification of §3.9, including the sole
nontrivial scored transport \(2\leftrightarrow3\).

**[THEOREM — exhaustive finite verification]** The verifier checks all 49
ordered pip-trump transports against called/powered membership, effective
incidence, led context, follow, and every pairwise contextual-order comparison.
It also confirms the three displayed class invariants.
...
**[FINITE VERIFICATION RECEIPT — stated corpus]** The independent verifier
...
#### 7.7.1 General finite capacitated matching kernel

A capacity-cell system is exactly a finite bipartite capacitated assignment
problem. Form the graph

\[
G_{\mathbf C}=(U\sqcup J,E),
\qquad
(d,s)\in E\iff d\in P_s,
\]

with demand one at every domino vertex and demand \(k_s\) at holder vertex
\(s\). A world in \(\Phi(\mathbf C)\) is a complete left-perfect
\(b\)-matching satisfying those holder demands.

Under this identification:

- Hall feasibility is capacitated matching feasibility;
- conditioning on \(d\to s\) is forced-edge contraction with holder demand
  reduced by one;
- exact marginal holder support is the set of edges belonging to at least one
  complete feasible matching;
- the one-witness SCC construction of §7.9 is the alternating-cycle
  characterization of those supported edges; and
- the determinate/binary/ternary normal form is the specialization of the
  matching-supported core to three holder vertices.

**[COROLLARY / STRUCTURAL SYNTHESIS]** The support theory may be formalized
first for finite capacitated bipartite matchings and instantiated afterward to
the three-hidden-seat Straight object. No game-specific theorem is weakened by
that generalization.
...
three-seat systems used for the Hall receipt, the independent verifier confirms
...
support, proves a 26-bit lower bound and a 46-bit outer-certificate ceiling for
...
**[BOUNDARY — standalone support is not a complete transition state]**
Membership in \(\mathscr R_{\mathrm{Str}}^m\) says that at least one legal
prefix realizes the support. A standalone support identifier does not retain
declaration, actor order, current trick, utility residue, or a realizing path.
The same support can occur in mechanically different states with different
legal actions. Consequently there is no exact **complete-game** transition
function on support identifiers alone. Section 7.14.1 proves the sharper
positive statement: when declaration and the full typed public observation
context are supplied, the exact successor **support** does factor through the
support normal form.
...
**declaration-tagged outer certificates** for that profile by
...
with the terminal profile assigned one canonical empty-void certificate per
declaration. These certificates enforce:
...
the lexicographically least outer certificate that decodes to the target
reachable support and ranks it within the displayed finite certificate set.

This is an upper bound, not an assertion that 46 bits are necessary or that the
certificate layout is transition-optimal. The same verified certificate count
gives useful context-relative ceilings:

- with the declaration supplied externally, fewer than \(2^{43}\) certificates;
...
These remain outer-certificate bounds, not exact minima. With the complete
...
#### 7.13.7 Symbolic support reachability

The complete-deal witness of §7.13.4 is exact but not minimal as a certificate.
The current support itself can carry the existential hidden world through the
public trace.

Fix a viewer hand, contract, declaration, initial leader, and an
actor-attributed public play trace. Start from unrestricted initial support on
the 21 unseen dominoes. Replay the trace using:

- the known viewer hand for viewer actions;
- the exact current support for hidden actions;
- the public trick residue for actor order, led context, and winner; and
- the typed support transition of §7.14.

A hidden action is accepted exactly when its conditioned successor support is
nonempty. A viewer action is accepted exactly when the domino is in the known
hand and is legal under follow-if-possible.

**[THEOREM — proved mathematically: symbolic trace equivalence]** An
actor-attributed public Straight play trace is accepted by this symbolic
support machine if and only if at least one complete deal legally realizes the
trace. The final symbolic support is exactly the rule-support fiber of that
trace.

**Proof.** For completeness, take a realizing deal. Inductively, its current
hidden remainder belongs to the exact support before every action. The legal
observed action therefore survives the typed conditioning, so every symbolic
successor is nonempty and still contains the deal's successor remainder.

For soundness, exact support after each accepted prefix is, by §7.5 and §7.14,
the set of current remainders compatible with that whole prefix. A nonempty
final support contains a current remainder world. The fixed-history bijection
of §7.5 adds each publicly played tile back to its actor and reconstructs one
complete initial deal realizing the entire trace. ∎

**[COROLLARY — proved mathematically]** An exact reachability certificate needs
no hidden deal. It may consist of the viewer's initial hand, the public
contract/declaration and initial leader, the actor-attributed public trace, and
the claimed final normal form. Replay verifies the claim.

**[COROLLARY — proved mathematically]** The reachable symbolic play/support
graph is finite and graded by total remaining tiles. Its support-output image
is exactly \(\mathscr R_{\mathrm{Str}}^m\).

This theorem does not give a closed-form support-only membership test. It gives
a smaller exact ancestry certificate and an intrinsic transition generator for
the reachable image.
...
#### 7.14.1 Exact support normal form as a transition state

Let \(N=\mathcal N(\mathbf C)\) be a feasible exact support normal form and let
\(\llbracket N\rrbracket\) denote its decoded fiber. For an observed hidden
play \(o=(s,d)\), define the compatible predecessor domain

\[
E_o=
\begin{cases}
\{\omega:d\in H_s(\omega)\},&\text{hidden lead},\\[1mm]
\{\omega:d\in H_s(\omega)\},&\text{successful follow},\\[1mm]
\{\omega:d\in H_s(\omega),\ H_s(\omega)\cap
\widehat\sigma_q^\delta=\varnothing\},&\text{slough in context }q.
\end{cases}
\]

For successful follow, the typed observation includes
\(d\in\widehat\sigma_q^\delta\); for slough it includes the opposite. Let
\(\vartheta_{s,d}\) remove \(d\) from seat \(s\).

**[THEOREM — proved mathematically: support-normal dynamic sufficiency]**
Given \(N\), declaration, actor, played domino, and the current led context or
lead boundary, the exact successor support normal form is uniquely determined:

\[
N'
=
\mathcal N\!\left(
\vartheta_{s,d}(\llbracket N\rrbracket\cap E_o)
\right).
\]

Viewer play leaves the hidden support normal form unchanged.

**Proof.** Exact decoding recovers the predecessor fiber. The typed observation
is a predicate on that fiber. The removal map gives exactly the successor fiber
by §7.14, and \(\mathcal N\) is the unique canonical representation of that
fiber by §7.10. ∎

The same update can be computed without enumerating worlds. Decode the normal
form to its reduced matching-supported holder graph. Then:

1. require the edge \(d\to s\);
2. for a slough, delete every edge \(e\to s\) with
   \(e\in\widehat\sigma_q^\delta\);
3. contract the forced edge by removing \(d\) and decrementing \(s\)'s quota;
4. compile the matching-supported core again; and
5. encode the resulting determinate/binary/ternary normal form.

**[THEOREM — proved mathematically: matching-minor update]** This
force/delete/contract/reduce operation equals the displayed fiber
conditioning and pushforward.

**Proof.** Deleting incident edges expresses the complete-void predicate,
forced-edge contraction expresses possession and removal of \(d\), and the
canonical reduction removes exactly the edges absent from every surviving
complete matching without changing the surviving matching set. ∎

Support alone is not a complete game state. The theorem requires declaration,
actor order, and led-context information from the public physical residue.

#### 7.14.2 Holder-edge monotonicity and the finite deletion budget

For every surviving hidden tile \(e\), define its exact marginal holder set at
time \(t\) by

\[
A_t(e)=\{s:e\in H_s\text{ in some world of }\Phi_t\}.
\]

**[THEOREM — proved mathematically: holder-edge monotonicity]** Within one
contracted deal attempt, after every accepted public play and for every
surviving hidden tile,

\[
A_{t+1}(e)\subseteq A_t(e).
\]

**Proof.** Take any successor world assigning \(e\) to \(s\). For a hidden
action, the inverse typed transition adds the fixed played domino back to its
fixed actor and produces a compatible predecessor world that still assigns
\(e\) to \(s\). For a viewer action, the hidden-remainder map is the identity.
Thus every successor holder edge existed before the action. ∎

Consequences:

1. a surviving certain tile remains certain;
2. new certain tiles may appear, but certainty never becomes ambiguity;
3. once a seat has zero residual ambiguity capacity it never re-enters the
   ambiguity component; and
4. the ambiguity tag moves only downward:

\[
\mathsf{Ternary}\to
\mathsf{Ternary},\mathsf{Binary},\mathsf{Determinate},
\]

\[
\mathsf{Binary}\to\mathsf{Binary},\mathsf{Determinate},
\qquad
\mathsf{Determinate}\to\mathsf{Determinate}.
\]

**[THEOREM — proved mathematically: 63-edge hand budget]** Before the first
play, the 21 unseen tiles each have three possible hidden holders, so the exact
holder graph has 63 edges. Every one of those edges disappears exactly once by
the end of the hand. At most

\[
21(3-1)=42
\]

edge deletions can occur while the corresponding tile remains live; the final
at-least-one edge for each tile disappears when that tile is played.

**Proof.** Edges never reappear. Every live tile must retain at least one
holder edge in nonempty exact support. Therefore at most two of its initial
three edges can disappear before removal, and all remaining incident edges
disappear at removal. The terminal hidden pool is empty. ∎

**[THEOREM — exhaustive finite verification]** On 1,331 distinct feasible
exact supports over canonical universes of sizes zero through four, the
reduced-kernel verifier checks 170,058 typed lead/follow/slough observations,
157,809 nonempty successors, 1,406,592 surviving tile-seat monotonicity tests,
and exact equality between extensional conditioning and the direct
matching-minor update.

**[FINITE VERIFICATION RECEIPT — stated corpus]** On 108 deterministic legal
complete hands spanning all nine declarations, symbolic support replay checks
all 3,024 public transitions. The exact holder graphs perform 6,804 total edge
deletions, exactly \(108\cdot63\), with no reappearance.
...
### 7.16 Folded physical play residue and the reduced viewer kernel

The evidence that produced exact support and the state required to continue
rule mechanics are different objects. If exact support is retained as the
dynamic support state of §7.14.1, completed-play attribution and public void
masks need not also remain in the physical/support kernel. They remain in the
retained evidence record whenever a policy, belief model, or utility reads
them.

#### 7.16.1 Exact current-trick fold

For declaration \(\delta\) and led context \(q\), let
\(C_{\delta,q}\) be the nonzero-tier dominoes, ordered by their distinct trick
keys. Let

\[
\operatorname{ord}_{\delta,q}:C_{\delta,q}\to
\{1,\ldots,|C_{\delta,q}|\}
\]

be that increasing ordinal. An unresolved current trick folds to
...
- \(q\) is the led context;
- \(r\) is the ordinal of the current maximum nonzero trick key;
- \(w\) is the seat currently winning; and
- \(z\) is the accumulated count value in the unresolved trick.

For a new play \((s,d)\), compute
\(r_d=\operatorname{ord}_{\delta,q}(d)\) when \(d\) is competitive and zero
otherwise. Replace \((r,w)\) by \((r_d,s)\) exactly when \(r_d>r\), and add
\(c(d)\) to \(z\).

**[THEOREM — proved mathematically: current-trick fold congruence]** Given the
remaining hands or exact viewer support, declaration, and actor order, two
unresolved trick prefixes with equal \((q,r,w,z)\) have identical future
follow legality, winner updates, final trick winner, and trick reward for every
common legal continuation.

**Proof.** Future follow legality consults only \(q\). Each new tile changes
the current winner exactly when its contextual key exceeds the stored maximum,
which is exactly the ordinal comparison. Count reward is additive in \(z\).
After the fourth play, \(w\) and \(1+z\) are therefore the exact winner and
trick points. No other property of the already removed losing tiles enters a
future rule. ∎

For every Straight declaration and leadable context,

\[
|C_{\delta,q}|\le13.
\]

The maximum is seven powered tiles plus six unpowered natural followers. Before
trick completion, \(z\in\{0,5,10,15,20,25\}\): at most two ten-point tiles and
one five-point tile can have appeared. On the fourth-play transition the local
sum may temporarily reach 30 before the trick is emitted and the fold is
cleared; that transient value is not an unresolved-state field. A transparent
field encoding therefore needs at most

\[
3+4+2+3=12
\]

bits for a declaration-local seven-valued lead-context index, 13-valued
strength ordinal, four-valued winner seat, and six-valued pending count. This
is a fieldwise exact bound, not a proof of globally minimal packing.

#### 7.16.2 Actor and leader from remaining capacities

At every unresolved legal trick, all four remaining-hand sizes take exactly two
values, \(h\) and \(h-1\). The low-capacity seats are precisely the seats that
have already acted and form one nonempty proper clockwise interval.

**[THEOREM — proved mathematically]** The four remaining-hand sizes uniquely
determine the current trick leader, number of plays made, and next actor when a
trick is open. At a trick boundary all four sizes are equal and the next leader
must be retained explicitly.

**Proof.** The unique low seat whose clockwise predecessor is high is the
leader. The number of low seats is the trick length, and the clockwise
successor of the low interval is the next actor. ∎

#### 7.16.3 Score conservation and utility-specific accumulators

Let \(H_m\) be the viewer's remaining hand, \(U\) the current hidden pool,
\(a\) the unresolved trick length, and \(z\) its pending count. The number of
completed tricks is
...
The total banked points across both partnerships are exactly
...
**Proof.** The unbanked count is exactly the count still in the four remaining
hands plus the count currently pending in the unresolved trick. Every completed
trick contributes one base point. ∎

Thus storing both partnership point totals is always redundant; one determines
the other. More importantly, the exact score residue is utility-relative:

- additive declaring points or signed differential use transition rewards and
  require no accumulated score in a memoized **continuation-return** state; an
  absolute hand-total value adds the already banked root offset outside that
  state;
- a point contract may retain only a capped remaining declaring threshold;
- a mark contract may retain only whether the sweep is still alive; and
- match utility retains the exact marks, shaker, and cross-hand residue required
  by that named lens.

No universal utility-independent minimal score accumulator is claimed.

#### 7.16.4 Reduced exact viewer play/support kernel

For a fixed contract, viewer frame, and named utility accumulator, an exact
within-hand physical/support kernel may be represented by

\[
K=(\delta,H_m,N,\tau,\alpha_U),
\]

where

- \(H_m\) is the known remaining viewer hand;
- \(N\) is the globally minimal exact hidden-support normal form;
- \(\tau\) is either a trick-boundary leader or the folded unresolved trick
  \((q,r,w,z)\); and
- \(\alpha_U\) is the exact utility-specific accumulator, possibly trivial.

**[THEOREM — proved mathematically: reduced-kernel sufficiency]** Relative to
the fixed contract and utility interface, \(K\) determines viewer legal actions,
acceptance and exact support transition for every public hidden action, actor
order, trick winner and reward, terminal hand detection, and the selected
utility accumulator transition.

**Proof.** Viewer legality follows from \(H_m\) and \(\tau\). Hidden-action
acceptance and support transition follow from \(N\), \(\delta\), and \(\tau\)
by §7.14.1. Actor order follows from remaining capacities and the boundary
leader by §7.16.2. Trick evolution follows from §7.16.1. The named accumulator
supplies exactly the utility residue not represented as transition reward. ∎

The ordered public path, bids, completed-play attribution, and raw void masks
remain in the separate continuation/evidence record \(e\) whenever a field or
utility reads them. They are not required twice in the physical/support kernel.
Conversely, an evidence-rich mechanical state may derive \(N\) rather than
store it. An exact implementation should select one semantic source and treat
the other as a derived or cached view; storing both as independent state invites
inconsistency.

The support component evolves by a finite matching-minor calculus:

\[
\text{force edge}
\;\to\;
\text{delete void-forbidden edges}
\;\to\;
\text{contract played tile}
\;\to\;
\text{canonical matching-supported reduction}.
\]

Together with holder-edge monotonicity, this makes the within-hand support
engine a monotonically deleting graph on at most 24 vertices and 63 initial
edges.
...
#### 12.7.1 Oriented-frame dihedral gauge

Introduce a coordinate orientation

\[
\eta\in\{+1,-1\},
\qquad
\operatorname{next}_\eta(s)=s+\eta\pmod4.
\]

Rotations preserve \(\eta\). A reflected frame

\[
f_k(s)=k-s\pmod4
\]

transports \(\eta\mapsto-\eta\), because
...
**[THEOREM — proved mathematically]** Rotations and reflected-frame maps form
an exact \(D_4\) coordinate gauge on the oriented family when every
orientation-dependent object is transported: actor order, seats,
partnerships, actions, evidence, policy types, and utility orientation.

This does not turn reflection into an automorphism of the fixed clockwise
extensive game. It permits an implementation to choose either oriented
coordinate frame while representing the same clockwise physical situation.
After a rotation anchor is fixed, reflection can provide at most one further
factor-two canonicalization; stabilizers may reduce the realized gain.

**[THEOREM — exhaustive finite verification]** The verifier checks all eight
oriented dihedral frames and confirms successor and opposite-partner incidence
under the transported orientation.
...
### 12.10 Mechanical future equivalence and global transition minimality

Minimality of a complete transition state is meaningful only after the required
observable outputs are named. Fix a finite reachable partial deterministic
machine with finite action alphabet \(A\). Its **output contract** may include
any
chosen combination of:

- action legality;
- physical outputs;
- exact support outputs;
- transition rewards;
- terminal labels; and
- utility-specific residue.

For state \(x\) and action word \(w\), let \(\operatorname{resp}(x,w)\) be the
complete legality/output trace, using a distinguished illegal symbol when a
prefix is not executable. Define
...
**[THEOREM — proved mathematically: mechanical Myhill–Nerode quotient]**
Future equivalence is a right congruence. The reachable quotient machine
\(X/{\equiv_{\mathrm{future}}}\) is, up to isomorphism, the unique smallest
deterministic exact machine realizing the selected output contract. Every
other exact deterministic realization factors onto this quotient.

**Proof.** Equality of the empty-word response gives equal current output.
Prefixing any common legal action preserves equality of all continuation
responses, so the relation is a right congruence and the quotient transition is
well defined. If an exact realization maps \(x,y\) to one representation state,
all future responses from that state are equal, hence
\(x\equiv_{\mathrm{future}}y\). Therefore each realization state maps to one
quotient class, giving a surjection onto the quotient. The quotient itself
realizes exactly the response map, so it attains the lower state-count bound.
Any two attaining machines are isomorphic through their response classes. ∎

The quotient depends on the output contract:

- a support-aware contract cannot merge states with different current exact
  support outputs;
- a purely physical contract may merge more;
- a utility-specific contract may be smaller still.

Thus there is no output-independent globally minimal “game state.” The reduced
kernel of §7.16 is an explicit exact sufficient representation. Equality with
a selected future-equivalence quotient is a separate finite theorem to be
proved or computed for that selected contract.

**[THEOREM — exhaustive finite verification]** The verifier enumerates all
5,898 total deterministic two-action, binary-output machines with at most three
states. Partition refinement agrees with an independent future-trace pair
search on all 17,560 unordered state pairs.
...
- the total support normal form is the coarsest exact deterministic semantic
  representation of capacity-cell support;
- exact support is also a closed dynamic support state when declaration and
  public action context are supplied;
- its holder relation is monotonically deleting, with a 63-edge initial budget
  and at most 42 informational deletions while tiles remain live;
- the current trick folds exactly to led context, current competitive strength,
  current winner, and pending count;
- open-trick actor order is recoverable from remaining capacities;
- score residue can be split from the physical/support skeleton and selected by
  utility;
- the symbolic support machine accepts exactly the legally realizable public
  traces and gives a deal-free exact reachability certificate;
- all pip-trump declarations form one unscored mechanics class, leaving exactly
  three unscored Straight classes;
- rotations plus orientation-transporting reflections give an exact \(D_4\)
  coordinate gauge on the oriented family; and
- for every selected finite output contract, future equivalence defines the
  unique globally minimal deterministic exact transition machine.

Not established:

1. that the explicit reduced viewer kernel of §7.16 is already equal to the
   future-equivalence quotient for any particular rich output contract;
2. one byte-minimal or runtime-minimal encoding independent of a named cost
   model and required operations;
3. the exact cardinality of the strictly Straight-reachable support image
   inside the proved 26–46-bit interval;
4. a closed-form local support-only reachability test replacing symbolic trace
   replay or exhaustive witness search;
5. a minimal retained evidence record, field-state representation, or augmented
   latent representation for arbitrary fields;
6. a universal minimal utility accumulator for arbitrary history-dependent
   utilities;
7. a general low-dimensional exact strategic quotient beyond the proved
   gauges and fixed-output future quotient;
8. unique beliefs at zero-probability histories without an assessment;
9. extension of the cell, support-normal-form, and reduced-kernel theorems to
   special contracts or altered information rules;
10. computational tractability of arbitrary predicate-restricted counting or
    enumeration;
11. a canonical probability measure or sampling law determined by support
    alone;
12. deterministic best-response existence for arbitrary added infinite
    private-signal models without stated measurability/selection assumptions;
13. algorithmic exact computability from finiteness alone when utilities or
...
be the match-global perfect-recall information state. The foundation now
separates three exact roles that a single implementation record may otherwise
conflate.

### Physical/support kernel

For a fixed contracted-hand problem, let

\[
K_{r,t}^m=
(\delta,H_{m,r}^t,N_{r,t}^m,\tau_{r,t},\alpha_{U,r,t})
\]

be the reduced exact viewer kernel of §7.16:

- stable declaration-relative physics \(\mathcal A_\delta\);
- the known remaining viewer hand \(H_{m,r}^t\);
- the globally minimal exact hidden-support normal form
  \(N_{r,t}^m\);
- the boundary leader or folded unresolved trick \(\tau_{r,t}\); and
- the exact named utility accumulator \(\alpha_U\), possibly trivial.

The support evolves by typed matching-minor reduction. Its holder edges only
disappear. Reachability is inherited from legal construction or certified by
symbolic trace replay; it is not a stored Boolean.

An evidence-rich mechanical state may instead retain played attribution and
void residue and derive \(N\). The reduced kernel retains \(N\) and discards
that provenance from the physical transition state. These are two exact
presentations of the same support semantics. They must not be stored as two
independent authorities. Derived caches are permitted only with dependency
validation.
...
be the required retained continuation record: exactly the viewer-known public
or private observation residue that the selected policy, field, or utility can
still consult beyond \(K\). It may contain the full relevant path. Hidden
actors' private residue remains in the augmented latent state rather than in
this viewer-known record.

Let \(\beta_{r,t}^m\) be the exact normalized measure on current hidden
remainders together with every coupled latent continuation-state component
that cannot be factored away. Rule support is the ambient physical domain;
\(\beta\) carries weights and may have smaller positive-mass support.

Relative to fixed rules, continuation field \(\Pi\), utility \(U\), and
allowed strategy class, the exact current decision state is

\[
B_{r,t}^m=(K_{r,t}^m,e_{r,t}^m,\beta_{r,t}^m).
\]
...
=
\mathcal V_U^\Pi
\left(K_{r,t}^m,e_{r,t}^m,\beta_{r,t}^m,d\right).
\]

### Native object

The resulting exact factorization is
...
with field and utility supplied as typed parameters to value.

The thesis remains unchanged: a domino is a stable physical node whose
strategic role is selected by declaration and by its relation to the whole
marked world. The refinement is sharper: the rule-support component has been
minimized semantically and made dynamic; the physical trick has been folded;
score has been separated by utility; and history is retained only in the
factor where history actually acts.

For any fixed finite output contract, the absolute deterministic transition
minimum is the future-equivalence quotient of §12.10. The displayed kernel is
proved exact; no unproved claim identifies it with that quotient.
